 ...
 * hay policy.
 *
 * Vị từ ấy không viết cứng gì cả: **mỗi tệp nguồn có lời gọi `.query(` phải đóng góp ít nhất MỘT
 * câu**. Nó bắt đúng lớp hỏng mà con số kia bỏ sót — bộ đọc mù một hình dạng chuỗi, một kiểu nối,
 * một tệp — và nó tự chặt hơn theo kho thay vì thiu dần.
 *
 * **[S1.113 / lượt soi ngang 78 — ③] Vị từ ấy đã DỜI sang `qt3-doc-sql.ts`** — cạnh chính bộ đọc,
 * cùng danh sách miễn trừ — vì lượt 78 đo ra rằng bên dùng THỨ HAI của bộ đọc này
 * (`qt3-ghim-schema.test.ts`) không có nó suốt sáu vòng, và hai con số viết cứng ở đó đã trôi
 * 32%/21%. Một bản vá ĐIỂM cho một lớp lỗi để lại đúng lớp lỗi ấy ở chỗ thứ hai.
 */
...
describe("[INV-H21] mỗi câu SQL sản xuất được PostgreSQL phân tích", () => {
  it(
    ...
    async () => {
      ...
      // [S1.113 / lượt soi ngang 78 — ③] Vị từ ấy nay sống ở `qt3-doc-sql.ts` cạnh chính bộ đọc,
      // vì bên dùng THỨ HAI của bộ đọc này đã không có nó suốt sáu vòng.
      expect(viPhamSanTheoTep(tatCa), "sàn theo tệp — tính đầy đủ của bộ đọc").toEqual([]);
