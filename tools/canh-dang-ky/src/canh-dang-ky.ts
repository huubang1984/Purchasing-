...
import { ListSubscriptionsByTopicCommand, SNSClient, type ListSubscriptionsByTopicCommandOutput } from "@aws-sdk/client-sns";

export interface SnsChiDoc {
...
export interface MongDoi {
  readonly topic: string;
  readonly bien: string;
  readonly nhan: readonly string[];
}

export interface DangKy {
  readonly giaoThuc: string;
  readonly diem: string;
  /** SNS ghi `PendingConfirmation` thay cho ARN khi chưa xác nhận. */
  readonly daXacNhan: boolean;
}

export type Hong =
  | { readonly loai: "cho xac nhan" | "khong co"; readonly bien: string; readonly viTri: number }
  | { readonly loai: "la"; readonly giaoThuc: string };

export interface KetQuaTopic {
  readonly topic: string;
  readonly soMongDoi: number;
  readonly hong: readonly Hong[];
}

export async function lietKeDangKy(sns: SnsChiDoc, topic: string): Promise<DangKy[]> {
  const ra: DangKy[] = [];
...
const cungDiaChi = (a: string, b: string): boolean => a.trim().toLowerCase() === b.trim().toLowerCase();

export function doiChieu(mongDoi: MongDoi, dangKy: readonly DangKy[]): KetQuaTopic {
  const hong: Hong[] = [];
...
const tenTopic = (arn: string): string => arn.slice(arn.lastIndexOf(":") + 1);

/** Các dòng log — hợp đồng với metric filter của stack 60 (`hinh-dang-canh-dang-ky.test.ts` so hai phía). */
export function dongLog(ketQua: readonly KetQuaTopic[]): string[] {
  const dong: string[] = [];
...
export function docMongDoi(giaTri: string | undefined): MongDoi[] {
  ...
  return ra.map((m: unknown) => {
    ...
    return { topic, bien, nhan: nhan as string[] };
  });
}

export async function kiemDangKy(sns: SnsChiDoc, mongDoi: readonly MongDoi[]): Promise<KetQuaTopic[]> {
  const ra: KetQuaTopic[] = [];
...
export async function handler(): Promise<{ diaChi: number; hong: number }> {
