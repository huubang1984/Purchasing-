...
export const MA_DUOC_PHEP_CHUA_PHU: ReadonlyMap<string, string> = new Map([
  ...
  // [S1.10.6] A2 ĐÃ ĐƯỢC GỠ — tiến trình `api` tồn tại và bộ quét rò rỉ chạy trên nó. Nguyên văn lý
  // do cũ, giữ để đối chiếu:
  ...
  // Ba vế đo được (phản hồi, header, log bắt được) và hai vế KHÔNG (heap, APM) ghi ở PHAM_VI_HEP.
  ...
  // [S1.10.2] E6 ĐÃ ĐƯỢC GỠ — nó có chủ ngữ: `ROUTES` của apps/api và ba header trên mọi phản
  // hồi. Nguyên văn lý do cũ, giữ để đối chiếu:
  ...
  // Phần còn thiếu (chính magic link dạng URL — S1.10.3) đi vào PHAM_VI_HEP, không ở đây.
...
export const PHAM_VI_HEP: ReadonlyMap<string, string> = new Map([
  ...
  // [S1.10.7, review lượt 2 H2-2] D2 nhận cờ §4 lần đầu: không vì cơ chế đổi, mà vì câu biện minh của
  // 030 rộng hơn cơ chế và reviewer đọc ra. Chú thích cũ ở dưới ("HAI mã còn lại (C3, D2) KHÔNG") giữ
  // nguyên văn — nó đúng tới ngày hôm nay.
  ...
  ["D5", "Được cưỡng chế cho đường đi **qua `requirePermission`**, và từ S1.68 cho lần từ chối ngoài nó **qua `throwAuditedDenial`** — cổng mở thầu (bốn vế, và từ S1.72 yêu cầu không tìm thấy), nhánh D2 của hai phép phê duyệt, và từ S1.72 bảng so sánh A4 cùng worker lúc giải mã. Một lần từ chối ở tầng CSDL (RLS/GRANT) không sinh bản ghi nào, một lần thử MFA thất bại **cố ý** không ghi sổ (ADR-008), và lần từ chối vì không tìm thấy ngoài cổng mở thầu và worker lúc giải mã chưa ghi sổ (khoản 133)."],
  ...
  ["D4", "**BA VẾ ĐẦU ĐƯỢC ĐO; VẾ *“không bao giờ im lặng”* CHỈ ĐƯỢC ĐO MỘT NỬA.** Đã đo: break-glass là một ĐƯỜNG RIÊNG (nó bỏ qua ngưỡng phê duyệt, và có test cho ca RFQ vượt ngưỡng vẫn đi qua); lý do là BẮT BUỘC ở tầng lược đồ; cảnh báo **BỀN** (một hàng `outbox_jobs` mang `severity: HIGH`, sinh trong CÙNG giao dịch tạo yêu cầu) và **TỨC THÌ** (`pg_notify` — test dùng một kết nối `LISTEN` và **không chạy một vòng poll nào**); cộng một lượt đột biến gỡ trigger cho thấy break-glass đi qua trong im lặng khi không có nó. **[S1.6, review an ninh HIGH-2a] MỘT LỖ ĐÃ ĐƯỢC BỊT Ở ĐÂY, VÀ NÓ NẶNG HƠN PHẦN CHÊNH:** tới trước vòng sửa, break-glass đi tới `APPROVED` với **KHÔNG một hàng phê duyệt nào** — `unseal_kiem_du_phe_duyet` trả về ngay, bảng cạnh cho phép, và `app_api` có `UPDATE (status, approved_at)` — nên MỘT câu lệnh viết tay hoàn tất chuyển trạng thái và chính người yêu cầu điều phối được nó. Một người sở hữu trọn chuỗi, và lớp bù duy nhất là một cảnh báo chưa ai tiêu thụ. 022 đòi một **NHÂN CHỨNG**: khác người yêu cầu, khác phiên — đúng hai vế `unseal_kiem_nguoi_duyet` đòi ở đường thường. Break-glass vẫn bỏ qua NGƯỠNG; nó không còn bỏ qua NGƯỜI THỨ HAI. **PHẦN CHÊNH:** **chưa ai TIÊU THỤ cảnh báo ấy.** Không handler nào đăng ký cho `BREAK_GLASS_UNSEAL_ALERT` và không tiến trình nào `LISTEN` trong sản phẩm; tệ hơn, một job không có handler bị `JobRunner` ghi thẳng `FAILED` với lý do `NO_HANDLER` và `onJobFailure` mặc định IM LẶNG. Hôm nay mệnh đề đúng ở mức *“tín hiệu đã được phát, bền và tức thì”*; nó CHƯA đúng ở mức *“có người biết”*, và khoảng cách ấy đòi một tầng vận hành mà `apps/` chưa có. **[S1.10.7, review lượt 2] BREAK-GLASS QUA HTTP HÔM NAY KHÔNG ĐI ĐƯỢC:** `POST /rfqs/:rfqId/unseal {breakGlass: true}` luôn 422, vì `requestUnseal` đòi `breakGlassWitnessSessionId` (phiên của người làm chứng, 022) mà route không có cách nào truyền. Vế *đường riêng* được đo ở tầng GÓI; ở tầng HTTP nó chưa tồn tại — và route không được đọc như đã hỗ trợ. **[S1.81 — ĐO] BA CÂU CỦA ĐOẠN \"PHẦN CHÊNH\" Ở TRÊN ĐÃ THIU, VÀ CÂU THỨ TƯ VỪA ĐỔI NGHĨA.** ⑴ *\"Không handler nào đăng ký cho `BREAK_GLASS_UNSEAL_ALERT`\"* SAI từ S1.6: `apps/unseal-worker/src/composition.ts` đăng ký nó, và `composition.int.test.ts` đo một job cảnh báo tới được adapter gửi kèm bản ghi `BREAK_GLASS_ALERT_DELIVERED`. ⑵ *\"khoảng cách ấy đòi một tầng vận hành mà `apps/` chưa có\"* là bản sao thứ SÁU của lời khai *\"`apps/` rỗng\"* (khoản 7, thiu từ S1.10). ⑶ *\"một job không có handler bị ghi thẳng FAILED với NO_HANDLER\"* nay chỉ còn đúng cho `kind` ĐÃ KHAI trong `KIND_KHONG_NGUOI_NHAN`: từ S1.81 câu claim lọc theo `kind`, nên một `kind` ngoài mảng lọc KHÔNG bị claim. **Và đây là phần đo được, nặng hơn cả ba câu trên:** tới S1.80 vế *\"có người biết\"* không chỉ CHƯA đúng — nó bị chính tiến trình `api` làm cho SAI. Runner của `api` (bảng handler đúng một khoá `LOGIN_LINK_SEND`) claim luôn `BREAK_GLASS_UNSEAL_ALERT` rồi ghi `FAILED` / `NO_HANDLER` ở lượt thử THỨ NHẤT, trước khi worker kịp chạm tới. Đo trên HEAD `e587819`: `status=FAILED attempts=1 ly_do=NO_HANDLER`. Một cảnh báo mức cao bị GIẾT chứ không phải bị bỏ quên. S1.81 vá vế ấy. ~~vế *\"có người biết\"* vẫn CHƯA đúng vì điểm vào tiến trình của worker chưa tồn tại (khoản 116), nhưng job nay nằm `PENDING` chờ — cứu được — thay vì `FAILED` không đường về.~~ **[S1.83 / lượt soi ngang 73 — ĐO] CÂU VỪA GẠCH THIU TỪ S1.82, tức từ vòng NGAY SAU vòng viết ra nó.** Khoản 116 ĐÓNG ở S1.82: `apps/unseal-worker` có `main.ts`, có `worker:dev`, hai pool riêng vai `app_unseal`, và nguồn danh sách tổ chức là `public.outbox_danh_sach_to_chuc()` (`052`, ADR-040). Nên vế *\"có người biết\"* nay đúng ở mức *\"có một tiến trình NHẬN và GỬI được cảnh báo\"* — `tien-trinh.int.test.ts` đo tiến trình lên được, thấy đủ tổ chức, và mốc chết hai chiều cho thấy worker nhặt đúng `kind` của nó trong khi `api` không chạm. **PHẦN CHÊNH CÒN LẠI, hẹp hơn hẳn phần chênh cũ:** không cụm nào đang CHẠY tiến trình ấy — kho chưa có tầng triển khai cho bất kỳ app nào (`git ls-files` không ra một Dockerfile, compose, systemd hay manifest nào), và `apps/api` cũng thế. Câu đúng hôm nay là *\"đường đã có và đo được, chưa ai bật nó ở một cụm thật\"*, không phải *\"điểm vào chưa tồn tại\"*; hai câu ấy khác nhau, và câu cũ khai RỘNG hơn về phía bi quan."],
...
export const MA_PHAI_CO_CO_HEP: ReadonlySet<string> = new Set([
  ...
  // [S1.10.6] A2 vào ĐÚNG hôm nó vào ô xanh: mệnh đề có năm vế, bộ quét đo ba. Gỡ cờ này là khai
  // "heap và APM đã được đo" — và chưa ai đo.
  "A2",
 ...
 *
 * [S1.17] HAI CÂU TRONG KHỐI B3 ĐÃ BỊ VÒNG S1.17 BÁC BỎ, và chúng KHÔNG được sửa tại chỗ: một
 * bản trích nguyên byte mà sửa được thì không còn là bằng chứng. Đính chính đi RIÊNG, ngay dưới
 * khối, ở `PHU_LUC_BAN_GIAO` — xem hằng số đó.
 */
/**
 * ĐÍNH CHÍNH cho khối trích nguyên văn ở trên. Nó là một hằng số RIÊNG, không phải một lần sửa
 * trong khối, và đó là điểm chính: bản trích là bằng chứng của trạng thái NĂM S0, còn đây là thứ
 * đã đo được ở S1.17. Trộn hai thứ vào nhau sẽ xoá mất chính cái đã thay đổi.
 */
export const PHU_LUC_BAN_GIAO: ReadonlyArray<{ ma: string; noiDung: readonly string[] }> = [
  {
    ma: "B3",
    noiDung: [
