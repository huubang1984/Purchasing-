...
**Ranh giới NÓI RA:**
- ⑴ Trần 5 s là trần của MỘT lần lấy kết nối, không của lần ghi: lần ghi đã có kết nối vẫn chờ khoá tư vấn của tổ chức tới `statement_timeout` 15 s (`createPool`; đo: 57014 — `lock_timeout` cũng 15 s nhưng đếm từ lúc bắt đầu chờ khoá, lượt soi 63b-4), giữ một kết nối `auditPool` và kết nối nghiệp vụ của người gọi.
...
- ⑺ `apps/unseal-worker` không có `auditPool`; lần từ chối của worker không ghi sổ — khoản 121 ⑶, không đổi.
...
# §S1.71 — khoản nợ 123: mọi lần ghi sổ chờ khoá ghi sổ của tổ chức tối đa 2 s — mệnh đề `SET lock_timeout = '2s'` trên chính hàm nối chuỗi (050, hardening mục (D1b)) ; `issueRfqKeyPair` bọc mọi cặp khoá và `extendRfqDeadline` xếp job trước lần ghi sổ đầu — khoản 126, 127 mở — lượt soi 65
...
- ⑸ Lần từ chối trong ca khoá bị giữ quá 2 s không vào sổ — D5 nhường khả dụng của tổ chức khác; hạn mức theo người gọi cho lần từ chối — khoản 122.
...
- ⑻ ⑼ ⑽ là trường hợp cụ thể của đánh đổi đã chọn — mọi lần ghi sổ chờ quá 2 s thì hỏng — nhưng câu hỏi trình chủ dự án ngày 2026-09-14 không nêu riêng chúng; chúng được đưa lại cho chủ dự án ở báo cáo cuối lượt làm việc.
