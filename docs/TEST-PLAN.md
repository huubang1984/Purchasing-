...
| **J3** | Người đề xuất award khác mọi người duyệt; và bộ ba *tạo RFQ · điều phối mở thầu · đề xuất award* không cùng một người. ~~**Phạm vi thật HẸP HƠN mệnh đề:** vế điều phối chỉ thấy lần điều phối ĐANG CHẠY — khoản **233**, rổ A~~ **[S1.124] Khoản 233 ĐÓNG:** vế điều phối đọc `unseal_dispatch_history` (`064`), nên thấy MỌI người từng điều phối, kể cả sau một lần điều phối lại | Trigger `award_kiem_de_xuat` (`061`, thân từ `064`) và `award_kiem_nguoi_duyet` (`061`), cùng trigger ghi lịch sử `unseal_ghi_lich_su_dieu_phoi` (`064`); cả ba ghim ở `hardening.always.sql` | T3 |
