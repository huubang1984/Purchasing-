...
| ID | Bất biến | Cưỡng chế | Tầng test |
|---|---|---|---|
| **F1** | Mọi truy vấn bị ràng buộc `org_id` ở tầng DB qua RLS, không chỉ tầng ứng dụng **[S1.32] — và RLS không được là một đường làm câu ghi trả 0 hàng im lặng (ADR-036): mọi policy RESTRICTIVE khai đủ bốn cột nguyên văn; mọi quyền SELECT/INSERT/UPDATE/DELETE đã cấp cho vai ứng dụng (đích danh hay qua PUBLIC) có policy PERMISSIVE phủ; bảng bật RLS ngoài tập tenant phải khai kèm lý do** **[S1.38] và ba tổng điều tra ấy nay có mục hardening (khoản 83⑴⑵⑶): mọi policy trên bảng RLS thuộc đúng một lớp — RESTRICTIVE ngoài khuôn 027 khai ~~sáu~~ **[S1.55; chữ sửa ở S1.66 — lượt soi ngang 59c NHẸ-12]** bảy cột nguyên văn ở hardening, bản test đòi khớp; phủ lệnh (~~miễn con của bảng tenant~~ **[S1.42, lượt soi 33b NẶNG-1]** KHÔNG miễn con/lá của bảng tenant — lượt soi 29 NẶNG-2 bỏ vế miễn trước khi S1.38 hợp nhất; câu sai sống bốn vòng ở sổ đăng ký và ma trận); RLS ngoài tenant khai — `migrate()` NÉM trên cụm đã deploy, và câu phán xét của hardening chạy trong chính test này** **[S1.40] con cháu MỌI BẬC của bảng tenant vào tập bật RLS (`LA_CUA_BANG_TENANT` đệ quy, khoản 84 — đo cháu hai bậc ngoài public: rò trước, fail-closed sau); bảng tạm kế thừa là của riêng phiên, không vào tập (đo)** **[S1.41] bảng có `org_id` ngoài public, ngoài tập tenant, không RLS phải khai (khoản 85 — kề với 83⑶; đo lỗ rò, NO INHERIT/DETACH hai đường thời gian, chuỗi RENAME); meta-test sentinel cho ~~năm~~ **[S1.47]** bảy danh sách khai rỗng của hardening** **[S1.43] bảng tenant đã khai (`BANG_TENANT_KHAI`, 29 tên kèm migration khai sinh — bản khai phải bằng tập theo tính chất) phải còn đúng tên, còn là tenant theo tính chất, mang chú thích neo theo oid: đổi tên cột/đổi tên bảng/chép bảng đè tên ⇒ `migrate()` NÉM (ADR-037)** **[S1.44] 83⑵ lấy tập vai = TÍNH CHẤT ∪ TÊN ĐÃ GHIM (`VAI_KET_NOI_UNG_DUNG` ∪ `ROLE_CANH`) và quyền theo kế thừa: vai lạ thành viên app_api + GRANT trực tiếp ⇒ bản bốn tên im; app_api_login bị gỡ membership + GRANT ⇒ bản chỉ-tính-chất im; quyền qua nhóm ⇒ bản grantee-trực-tiếp im (khoản 88, lượt soi 36); tập vai kết nối ứng dụng theo `'USAGE' OR 'SET'` — hồ sơ N3 (cụm trống, vai deploy CREATEROLE chạy `migrate()` đầu tiên) đi qua mọi migration và dừng ở phán xét với một mục có tên** **[S1.46] bảng KHÔNG có `org_id` nhưng có khoá ngoại một cột — của nó hay của tổ tiên INHERITS — tới bảng tenant theo tính chất (đích khai triển từ `MAU_VI_TU_BANG_TENANT`: `organizations` lẫn `users`…), ngoài tập tenant, không RLS phải khai (khoản 86 nửa gốc — đo lỗ rò ở `k` LẪN `public`, `migrate()` NÉM cho cả hai; có `org_id` ⇒ 85, bật RLS ⇒ 83⑶; ranh giới nói ra: cột uuid trần, khoá ngoại nhiều cột, bậc kế trên đồ thị khoá ngoại; cửa ra 83⑶ yếu — khoản 91)** **[S1.50] cửa ra ấy hết yếu: MỌI view/matview của lược đồ dự án phải `security_invoker` (chuỗi view lồng và view đọc qua hàm lọt mọi vế đích — đo), và hardening FORCE mọi bảng bật RLS; đo dưới chủ bảng KHÔNG superuser — bảng chỉ ENABLE ⇒ app_api đọc trọn qua view, sau FORCE ⇒ hết rò** **[S1.47] GUC `app.*` gắn sẵn cho phiên ứng dụng bị phán xét theo tính chất — mức database, ALTER ROLE ALL, vai kết nối, chính phiên deploy (ALTER SYSTEM/postgresql.conf/options=), `pg_parameter_acl`, `proconfig` hàm (khoản 87 — đo 42501 dưới vai deploy thường, RESET ALL im lặng placeholder, GRANT SET ON PARAMETER; không tự RESET); `withTenant` từ chối phục vụ khi mặc định phiên của bốn GUC bị đầu độc và xoá ba GUC khách trong mọi giao dịch (đo trên `user_login_tokens`)** **[S1.48, lượt soi ngang 40] `withTenant` phân biệt rò phạm vi phiên từ mã ngoài nó (RESET rồi đọc lại — huỷ kết nối) với mặc định phiên (giữ); `migrate()` từ chối trước lượt sửa khi phiên deploy mang GUC `app.*` (migration tạm không chạy); khoản 86 nhận cả khoá ngoại hợp thành; mọi chiều ngược đọc catalog bằng JOIN (vai không USAGE vẫn đánh giá được); tập tên GUC ⒞ có census literal migrations ⊆ tập; `_khach` đếm từ catalog theo số bảng tenant** **[S1.51] Ba GUC VẬN HÀNH gắn sẵn cho phiên từ nguồn ngoài mức database bị phán xét ba nhánh (khoản 92): catalog `pg_db_role_setting` miễn nhiễm ƯU TIÊN NGUỒN (một hàng ưu tiên cao che hàng thấp — đo), phiên deploy hai vế `reset_val`/`setting` (đo: `SET` trong phiên không đụng `reset_val`), dòng khai thiu; `migrate()` đọc HAI LẦN — trước lượt sửa (bỏ qua nguồn `database` để ba mục kề còn tự chữa được) và ngay sau lượt sửa (chặn vòng migration đánh số, đòi kết nối mới); `ALTER SYSTEM SET search_path` chặn ở phép đọc TRƯỚC lúc ghim search_path (chỉ với nguồn áp cho MỌI phiên — `rolconfig` của vai deploy cố ý đứng ngoài, `[CR1]`/`[Minor]` ghim). **[S1.51 / lượt soi 44]** `migrate()` chụp hàng mức database TRƯỚC và SAU lượt sửa: hàng che mang GIÁ TRỊ ĐÚNG bị lượt sửa gỡ ⇒ deploy DỪNG đúng lượt ấy rồi lượt sau đi thẳng (kỳ vọng LẬT có chủ đích ở `[fix round 5]` và `[khoản nợ 83⑷⑸⑧]`); ba GUC xét theo TÍNH CHẤT nên `search_path = 'public'` và `session_replication_role = local` không bị chặn oan; thông điệp deploy chỉ mang TÊN GUC, không mang giá trị** **[S1.52] Khoản 95: `pg_parameter_acl` (ALTER SYSTEM trên ba tên, SET chỉ trên tham số SUSET) và `proconfig` của hàm (search_path lạ, pg_catalog nêu sau, replica, row_security=off) bị phán xét; `GRANT SET` trên tham số USERSET, `pg_catalog, pg_temp`, `"$user"`, `pg_catalog, "$user"`, `row_security = true` không bị chặn oan; tính chất search_path cấm pg_catalog đứng sau ở cả phiên lẫn hàm (đo cướp `lower()`)** **[S1.53] Khoản 94: sau FORCE, mọi lệnh của chủ bảng không superuser trên bảng RLS phải có policy PERMISSIVE phủ vai chủ — đo 0 hàng không lỗi dưới chủ thường khi policy chỉ `TO app_api`; phủ qua nhóm được tính, đúng lệnh ấy và không hơn; bảng con của cha bật RLS và lệnh chủ bảng đã tự REVOKE đứng ngoài; RESTRICTIVE không tính là phủ; census độc lập chủ bảng thay khẳng định rỗng ruột** **[S1.54] Khoản 96: mã của lược đồ dự án ghi ba GUC vận hành vào phiên người gọi bị phán xét — thân hàm (set_config, SET/SET LOCAL/SESSION, RESET, RESET ALL, UPDATE pg_settings, chuỗi EXECUTE nguyên văn; hoa/thường, có nháy, chú thích giữa token kể cả lồng, tên dollar-quote và U&), BEGIN ATOMIC, policy USING/WITH CHECK, DEFAULT, CHECK của bảng và domain, view, trigger WHEN; đọc GUC, set_config GUC khác, mệnh đề SET, extension, pg_temp không bị nêu; khai miễn đúng tên và không thiu; tên dựng lúc chạy là ranh giới ghim bằng test; `withTenant` không commit dưới replica (đo: khoá ngoại bị bỏ qua trong giao dịch ấy, không hàng nào được ghi) và huỷ kết nối nhiễm replica, row_security hay search path hiệu lực — kể cả nhiễm từ trước giao dịch và khi phép đọc lại ném; bảng tạm dưới vai có TEMP và `local` không bị huỷ oan** **[S1.55] Khoản 98: policy trên bảng RLS ngoài `public` — PERMISSIVE, RESTRICTIVE, và RESTRICTIVE đúng khuôn 027 — có đường khai ở 83⑴ kèm lược đồ: chưa khai thì bị nêu, khai thì im; dòng khai cho một lược đồ không che bảng cùng tên ở lược đồ khác, kể cả bảng cùng tên với bảng đã khai ở `public`; gỡ policy thì dòng khai thiu nêu đúng lược đồ; bảng khai không tồn tại thì không nêu; vai thật tên PUBLIC viết hoa không khớp cột vai PUBLIC; policy một vế khai được bằng NULL; PERMISSIVE trên dữ liệu tenant mà [CR1] không soi — lá phân vùng ngoài `public`, con của bảng gốc, bảng có org_id — không khai được; bản test và bản hardening của hai danh sách khai và của biểu thức vai khớp** **[S1.56] Khoản 97: mục 94 soi thêm vai chạy migration — `current_user` của phiên phán xét — khi RLS áp cho nó theo gương `check_enable_rls`: vai không phải chủ, hay thừa kế chủ trên bảng FORCE (kể cả khi chủ là BYPASSRLS hay đã tự REVOKE), có quyền mà không policy nào phủ nó đọc và ghi 0 hàng không lỗi, và mục nêu đúng các lệnh SELECT/UPDATE/DELETE vai ấy có quyền, kể cả GRANT mức cột; thành viên thừa kế chủ trên bảng NO FORCE, superuser, BYPASSRLS, chính chủ, bảng tắt RLS, bảng thuộc extension, bảng con của cha bật RLS và vai thiếu USAGE lược đồ thì im; INSERT không xét vì ném; policy phủ theo lệnh và qua nhóm INHERIT, nhóm NOINHERIT và RESTRICTIVE không phủ; hồ sơ N2 — migrate() dưới vai deploy NÉM và nói backfill của chính lượt đã ghi checksum, thu hồi quyền thì đi qua; ranh giới có ghim — bảng tenant chuẩn không EXECUTE thì ồn** **[S1.57] Khoản 100: chủ thể vai chạy migration của mục 94 còn được hỏi TRƯỚC vòng đánh số khi còn tệp chưa áp — hồ sơ N2 với một backfill đang chờ: migrate() dưới vai deploy từ chối trước khi tệp nào chạy, tệp không được ghi checksum, hàng giữ nguyên, thông điệp nêu bảng/vai/lệnh kèm đường tới quyền (cấp thẳng, qua nhóm), chạy dưới superuser thì backfill áp đủ; dòng mà một migration dưới chính vai deploy sửa được — chính chủ bảng, thành viên INHERIT của chủ thường và của chủ BYPASSRLS, vai có ADMIN OPTION trên chủ, vai tự cấp membership nhóm mang quyền — không bị chặn trước vòng và migration vá lỗi tới đích, còn membership nhóm do superuser cấp kèm ADMIN thì vẫn bị chặn; không tệp chờ thì không hỏi trước vòng; lỗi khác TP100 ở lượt hỏi trước vòng nổi nguyên; tệp kết thúc dưới vai khác vai mở vòng (SET LOCAL ROLE, SET ROLE, tệp tự COMMIT rồi SET ROLE) bị ROLLBACK, không ghi checksum, kết nối bị huỷ, và tệp ném lỗi sau khi commit một SET ROLE cũng huỷ kết nối; ranh giới ghim — tệp đổi vai rồi RESET ROLE thì đi qua, phần tệp trước lần COMMIT của chính nó đã commit; lớp tĩnh — migration của kho không viết thẳng câu đổi vai** **[S1.58] Khoản 101: policy phụ thuộc `app_current_org_id()` thôi tính là phủ vai chạy migration có EXECUTE trên hàm ấy mà RLS không coi là chủ — hồ sơ N2 với một backfill đang chờ: vai deploy có SELECT, UPDATE và EXECUTE do superuser cấp ⇒ migrate() từ chối trước vòng, tệp không được ghi, hàng giữ nguyên, thông điệp nêu đường tới EXECUTE và lối ra theo đường; gỡ EXECUTE ⇒ backfill ném 42501 và không được ghi; vai deploy là thành viên app_api ⇒ chặn trước vòng và không lời khuyên nào thu hồi khỏi app_api; EXECUTE qua nhóm tự cấp ⇒ không chặn trước vòng, backfill cùng lượt bị tiêu (ranh giới ghim), migration vá lỗi dưới chính vai deploy tới đích; ở mức mục: không EXECUTE thì policy tenant chuẩn vẫn tính là phủ, có EXECUTE thì nêu hai lệnh đã cấp; ba đường tự cắt EXECUTE không bị chặn trước vòng, EXECUTE cấp thẳng hay qua nhóm do superuser cấp thì bị chặn; policy không phụ thuộc hàm vẫn phủ, hình dạng khác phụ thuộc hàm vẫn bị loại, dòng khoản 97 không được tha nhờ vế EXECUTE, ADMIN trên vai sở hữu bảng không tính; ranh giới ghim (khoản 102) — vai deploy là chủ bảng FORCE hay thừa kế vai sở hữu bảng và hàm thì backfill bị tiêu và deploy xanh, kiểm khoá ngoại ban đầu dưới chủ FORCE có EXECUTE đánh dấu ràng buộc hợp lệ mà không kiểm hàng** **[S1.59] Khoản 99: mỗi lần lấy client của pool có vai từ chối kết nối không sạch — `row_security = off`, replica do hàm SECURITY DEFINER để lại (cả đường `pool.query`), search path phạm vi phiên, giao dịch bỏ ngỏ ⇒ lời gọi ném `KetNoiNhiemError`, kết nối bị huỷ (đo pid), lần sau là kết nối sạch; mặc định phiên `row_security = off` của vai đăng nhập ⇒ lần lấy đầu ném, chẩn đoán mặc định phiên; SET LOCAL, replica `local` và mặc định phiên search path hợp lệ khác mặc định máy chủ giữ kết nối; ranh giới ghim — DDL đổi search path hiệu lực làm mỗi kết nối bị huỷ một lần; hai bộ dọn nền không commit dưới replica do một trigger đặt giữa câu dọn (hàng cũ còn nguyên, kết nối bị huỷ), lỗi COMMIT khác TP096 đi ra nguyên dạng; census: mọi đường chạy SQL trên pool ngoài withTenant, mọi `createPool` sản xuất có vai, mọi COMMIT có khối chặn — khai theo số lượng kèm lý do** | **Postgres RLS** + `db/rls-coverage.int.test.ts` + `db/migrations/hardening.always.sql` | **T3**, T5 |
...
> **[S1.115 / khoản 229] Nhóm này vào sổ SAU KHI từng hàng được ĐO, không cùng lúc với spec.**
...
| ID | Bất biến | Cưỡng chế | Tầng test |
|---|---|---|---|
| **J1** | Con số xếp hạng chỉ gồm khoản có đơn vị TIỀN; một điểm phi giá không bao giờ đi vào `effective_cost` | Hàm thuần, `CHECK` cấu trúc và trigger nội dung `rfq_evaluation_lines_kiem_thanh_phan` (`057`) | T1, T3 |
| **J2** | Mỗi hàng xếp hạng **tái lập được**: tính lại từ `components` cộng phiên bản chính sách ra ĐÚNG `effective_cost` đã lưu | Hàm thuần, **và một bản cài ĐỘC LẬP trong bộ bằng chứng** mà `depcruise` `g17-` cấm chạm `packages/danh-gia` — ADR-059 | T1, T3 |
| **J3** | Người đề xuất award khác mọi người duyệt; và bộ ba *tạo RFQ · điều phối mở thầu · đề xuất award* không cùng một người. ~~**Phạm vi thật HẸP HƠN mệnh đề:** vế điều phối chỉ thấy lần điều phối ĐANG CHẠY — khoản **233**, rổ A~~ **[S1.129] Khoản 233 ĐÓNG:** vế điều phối đọc `unseal_dispatch_history` (`064`), nên thấy MỌI người từng điều phối, kể cả sau một lần điều phối lại | Trigger `award_kiem_de_xuat` (`061`, thân từ `064`) và `award_kiem_nguoi_duyet` (`061`), cùng trigger ghi lịch sử `unseal_ghi_lich_su_dieu_phoi` (`064`); cả ba ghim ở `hardening.always.sql` | T3 |
...
| **J7** | Một RFQ có **tối đa MỘT** award còn sống | Trigger `award_kiem_mot_award_song` đọc hàng trạng thái MỚI NHẤT dưới khoá tư vấn (`061`, thân từ `068`) — không phải chỉ mục UNIQUE bộ phận, vì bảng chỉ-ghi-thêm giữ hàng cũ | T3 |
...
- **[S1.35] Bốn giới hạn lượt soi ngang 25 ĐO ĐƯỢC, chưa đóng** (khoản nợ ~~79,~~ ~~80~~ **[S1.60] đóng**): ~~một trigger canh
  ...
  `pg_stat_activity.usename`, `is_superuser`, `session_authorization`, `pg_get_userbyid`).~~ **[S1.60] hai giới hạn ấy đóng (khoản
  80):** vế ⒠ so giá trị đã khai của mọi hàng, và nhân chứng chỉ hợp lệ dưới vai ứng dụng trừ năm cặp khai. Giới hạn còn lại của vế ⒠:
  giá trị máy chủ sinh chỉ khai `KHAC_NULL`; UPDATE không đổi giá trị không so được; cột không khai (khác nhau giữa các hàng, hay vai
  nhân chứng không SELECT được) không được so.
