...
**Phần KHÔNG đóng.** ~~Hình dạng "câu ghi hàng đứng sau lần ghi sổ đầu" còn ở `approveUnseal` — khoản 140, đọc, chưa đo.~~ **[S1.76] Khoản 140 ĐÓNG vì TIỀN ĐỀ SAI:** đo được rằng `approveUnseal` không bao giờ chờ khoá hàng trong khi giữ khoá ghi sổ — câu `INSERT INTO public.unseal_approvals` mang khoá ngoại tới `unseal_requests` nên nó đã tuần tự hoá trên hàng ấy TRƯỚC lần ghi sổ đầu. Ba kịch bản người giữ, kể cả người giữ nhẹ nhất (`dispatchUnseal`, không chạm `status`), đều cho cùng kết quả (§S1.76). ~~Ngưỡng khoá MFA vẫn phụ thuộc lần ghi sổ `MFA_LOCKED` nên trong lúc khoá ghi sổ bị giữ thì nó không chạm — khoản 139, đo.~~ **[S1.75] Khoản 139 ĐÓNG:** lần ghi `MFA_LOCKED` nay nằm trong một SAVEPOINT, nên khoá ghi sổ bị giữ thì ngưỡng VẪN chạm (đo). Dư lượng `40P01` / `57014` là khoản 143. Trần 2 s không đuổi người giữ; khoản 128 vẫn mở.
