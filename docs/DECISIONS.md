...
**Ngày:** 2026-09-26 · **Trạng thái:** **Đã chấp nhận** (chủ dự án chọn trong lượt soi hình dạng S1.139) ·
Liên quan: **ADR-043**, **ADR-058** ⑷, ADR-065, ADR-066, ADR-082 · Spec: `docs/superpowers/specs/2026-09-26-trustprocure-s3-kiem-soat-mua-sam.md` §2.3 (a)
...
S1.139 đo thêm một sự thật đổi hình dạng câu hỏi: `master` **đang là nguồn triển khai thật** — PR #132 (SES, ADR-065)
...
*"không chạm mảnh nào của `PRODUCT.md` §11; chạy dưới công tắc ADR-080"*.
...
Supplier Qualification vào S3, rồi ở lượt soi chọn **chỉ đếm nhà cung cấp đã thẩm định** vào ngưỡng K2 (ADR-082 ⑹iii) —
...
- Mỗi nhà cung cấp mới cần **một người thứ hai bên mua** xác minh trước khi được đếm. Ma sát rơi vào bên mua, không rơi vào
  nhà cung cấp — đó là cách duy nhất để ADR-082 ⑹iii và `PRODUCT.md` §8 ⑴ cùng đúng.
...
## ADR-082 — Lượt soi hình dạng spec S3: năm quyết định của chủ dự án, và những gì lượt soi chốt từ tiền lệ

**Ngày:** 2026-09-26 · **Trạng thái:** **Đã chấp nhận** · Vòng: S1.139 · Liên quan: ADR-017, ADR-020 [S1.70], ADR-023,
ADR-043, ADR-050 (khuôn), **ADR-051**, ADR-054, **ADR-058**, ADR-060, ADR-080, ADR-081 · Biên bản:
`evidence/security-reviews.md` §S1.139

**Bối cảnh.** Lượt soi S1.139 chạy bốn góc độc lập trên bản nháp spec S3 — lời khai và mâu thuẫn nội tại, khả thi cưỡng chế
...
- (iii) chỉ đếm nhà cung cấp có xác minh còn hiệu lực (ADR-081 ⑵);
...
- `rfq_bam_noi_dung` giữ nguyên. Nếu định nghĩa lại nó để phủ danh sách, mọi UPDATE sau đó trên một gói cấp kép đang OPEN
  ...
  **[S1.140]** Cơ chế ấy đã sửa (`067`: khối đếm chỉ chạy ở cạnh vào OPEN). Chốt giữ nguyên: đổi định dạng băm vẫn vô hiệu chữ
...
- Nó không nới dải nhãn `[A-HJ]`→`[A-HJK]` — phần còn lại của S3.0 theo spec §9~~, làm cùng K1 ở S3.1~~. **[S1.153]** Vế vừa
  gạch trái spec §9, vốn đặt việc nới dải TRƯỚC hạng mục đầu; chỉ phép kiểm `"ABCDEFGH"` chờ K1. Dải đã nới ở S1.153.
...
## ADR-090 — Số hiệu cấp lúc merge: số tạm trên nhánh, `pnpm cap-so` cấp số thật
