...
| **J6** | ~~Mọi lần đề xuất, duyệt, huỷ award, và mọi lần từ chối của cổng đánh giá, đều để lại một hàng sổ~~ **[S1.116 / khoản 239 / ADR-060] PHÁT BIỂU LẠI ĐÚNG MỨC, sau khi đo được rằng nửa sau CHƯA BAO GIỜ ĐÚNG:** mọi lần đề xuất, duyệt, huỷ award để lại một hàng sổ; mọi lần từ chối QUYỀN để lại một hàng `PERMISSION_DENIED`; và mọi lần từ chối TRẠNG THÁI **nói rằng người dùng đi một bước của chuỗi không đúng thứ tự** để lại một hàng `RFQ_STATE_DENIED`. Từ chối nói rằng CẤU HÌNH chưa sẵn sàng thì **KHÔNG** — luật và cái giá của nó ở ADR-060. Giới hạn còn lại, nói ra: từ chối do TRIGGER huỷ giao dịch nên không lối nào ghi được | Khuôn D5, cộng `packages/danh-gia/src/tu-choi-vao-so.ts` — bảng `VAO_SO` là một `Record` ĐẦY ĐỦ nên một mã mới không có dòng quyết định KHÔNG biên dịch được |
...
| S2.7 | Bằng chứng tái lập | ~~Bộ xuất mang đủ đầu vào để tính lại; đây là vế cuối của điều kiện hoàn thành~~ **[S1.114] XONG** — `tools/bo-xuat-danh-gia` theo **ADR-059**: bundle tự đủ + `DAC-TA.md`, kiểm bằng HAI lớp trong đó lớp chịu lực là một bản cài ĐỘC LẬP (`depcruise` `g17-`), và **J2** nay có cả vế khó. Ô ma trận cho J2 mở ở vòng **229**, không ở vòng này |
