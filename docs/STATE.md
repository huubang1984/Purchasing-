...
| 162 | **[MỞ]** ~~**[S1.82 — ĐO] MƯỜI CHỖ TRONG BẢY MIGRATION**~~ **[S1.83 / lượt soi ngang 73 — ĐẾM LẠI] MƯỜI HAI CHỖ TRONG TÁM MIGRATION ĐÃ ÁP KHAI *“mục (C) CẤM mọi SECURITY DEFINER”*, VÀ TỪ `052` CẢ MƯỜI ĐỀU THIU — KHÔNG CHỖ NÀO SỬA ĐƯỢC.** ~~Đếm được: `005` (190, 329, 460), `006` (110, 401), `010` (231), `011` (249), `018` (253), `027` (116), `034` (14).~~ **[S1.83] Phép đếm ấy BỎ SÓT HAI chỗ, và con số sai được chép ra bốn nơi trong cùng một vòng.** Đếm lại bằng `grep -n "SECURITY DEFINER" db/migrations/0*.sql` rồi đọc từng câu: `005` (190, 329, 460), `006` (110, **246**, 401), `010` (231), `011` (249), `018` (253), **`020` (68)**, `027` (116), `034` (14) — **12 chỗ, 8 migration**. Hai chỗ sót đều nói ra lệnh cấm chứ không chỉ mô tả hình dạng: `006:246` *“thu hẹp nó xuống một hàm SECURITY DEFINER là thứ mục (C) của hardening CẤM”*, `020:68` *“KHÔNG SECURITY DEFINER (hardening §C cấm)”*. Bài học của chính khoản này: một phép đếm viết tay là một lời khai, và nó thiu theo đúng cách mọi lời khai khác thiu. Sửa chú thích của một migration ĐÃ ÁP là đổi checksum của nó và làm `migrate()` từ chối trên mọi cụm đã deploy — khoản 19. Nên đính chính chỉ đặt được ở NGOÀI: ADR-040, hàng này, và khối mở đầu của `052`. **Một chỗ THÌ sửa được và đã sửa:** `hardening.always.sql` tự khai *“sáu migration”* — sai, đúng ra BẢY; tệp `.always` chạy lại mỗi lượt nên không có checksum khoá. **Vì sao vẫn MỞ:** không lớp nào đối chiếu chú thích của migration đã áp với trạng thái thật, nên lớp lời khai thiu này sẽ lặp lại ở mọi ngoại lệ hardening tương lai. Cùng họ với khoản 151. Hình dạng đề xuất: một cổng đọc `db/migrations/*.sql` tìm các câu khai TUYỆT ĐỐI (*“CẤM mọi …”*, *“hiện RỖNG”*, *“không có …nào”*) rồi đối chiếu với danh sách ngoại lệ thật của hardening. §S1.82. | `db/migrations/hardening.always.sql`, `docs/DECISIONS.md` |
