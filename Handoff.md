...
**[S1.79] Cập nhật 2026-09-18 · S1 (Sealed Bid Core) CÓ MÃ TRỌN.** `master` ở `39f6b7c`, 403 commit,
~~76~~ **[S1.115] 112** pull request đã merge, cổng evidence ~~56/56~~ ~~**62/62**~~ **[S1.116] 63/63** bất biến. Câu bị gạch ở trên **sống từ 2026-08-29 tới
S1.78** — qua 94 commit sửa chính tệp này, và không lần nào ai chạm dòng ấy. Cùng lớp lỗi với lời khai
*"chưa có mã nguồn"* mà lượt soi ngang 72 tìm ra ở `docs/ARCHITECTURE.md`; lượt ấy bắt được cả bản này,
S1.79 vá.
...
| HEAD của `master` | ~~**`30d1972`** (merge commit, 2026-08-29) — 49 commit~~ **[S1.79] `39f6b7c`** (merge PR #78, 2026-09-18) — 403 commit |
| Nhánh gốc | `master` |
| Số commit trên nhánh S0 | **46** (giữ nguyên trong lịch sử, **không** squash) |
| Thay đổi so với `master` trước merge | 105 file, +36 375 / −88 |
| Pull request (S1) | [#2](https://github.com/huubang1984/Purchasing-/pull/2) — ~~**MỞ**~~ **[S1.79] MERGED** 2026-09-06 (merge commit `b1a9a8b`), 15 commit, XANH cả bốn job ở run `33704750680`. Tiêu đề và mô tả đã được viết lại 2026-09-03: bản cũ nói "kế hoạch S1" trong khi PR chứa trọn S1.1–S1.3, sáu ADR và chín migration |
...
**[S1.140] 67 migration đánh số** (~~**[S1.132] 66**~~, ~~**[S1.130] 65**~~, ~~**[S1.129] 64**~~, ~~**[S1.119] 63**~~, ~~**[S1.118] 62**~~, ~~**[S1.110] 61**~~, ~~**[S1.109] 60**~~, ~~**[S1.108] 59**~~, ~~**[S1.105] 58**~~, ~~**[S1.102] 56**~~, ~~**[S1.100] 55**~~, ~~**[S1.96] 54**~~, ~~**[S1.85] 53**~~, ~~**[S1.82] 52**~~, ~~**[S1.76] 51**~~, ~~**[S1.71] 50**~~, ~~**[S1.61] 49**~~, ~~**[S1.28] 48**~~) + một lớp cưỡng chế chạy ở mọi lần `migrate()`. Con số ấy
...
db/migrations/001_roles_and_functions.sql     roles, hàm nền
             ...
             008 … 056                        S1 — xem db/migrations/
             hardening.always.sql             tự-sửa hoặc phán-xét, chạy MỌI lần
```

**[S1.66 / lượt soi ngang 59c NHẸ-13]** Dòng `008 … 048` của khối trên đã sửa thành `049` — S1.61 thêm migration 049.

**[S1.72 / lượt soi ngang 66c-3]** Dòng `008 … 049` của khối trên đã sửa thành `050` — S1.71 thêm migration 050; lần sửa tay thứ hai của cùng dòng — khoản 136.

**[S1.100]** Dòng `008 … 054` của khối trên đã sửa thành `055` — S1.100 thêm migration `055`, và đây là **lần đầu dòng ấy được sửa TRONG CHÍNH VÒNG thêm migration** chứ không phải bởi một lượt soi sau đó. Khoản 136 vẫn mở: một lần không thiu không phải một lớp.
...
~~**Bảy gói + hai công cụ:**~~ **[S1.142] 14 gói + 9 công cụ** (~~**[S1.123] 8**~~ ~~**[S1.114] 7**~~ ~~**[S1.104] 6**~~ ~~**[S1.89] 13**~~ ~~**[S1.28] 5**~~ — cùng khuôn với lời khai migration ở trên: lịch sử giữ con số trần, vì mẫu của cổng khớp CỤM ĐẦY ĐỦ và hai cụm đầy đủ làm phép đột biến của chính nó rỗng ruột) — cũng suy từ `git ls-files`:
...
| `tools/cap-so` | `pnpm cap-so`: cấp số thật cho số tạm của nhánh lúc merge, viết lại lời khai đếm; `--kiem` là cổng CI (ADR-084) |
...
- ~~**`apps/` RỖNG.** Không một đường gọi sản phẩm nào tới `listOrganizations`, `start()` của
  ...
  **[S1.21] SAI TỪ S1.10** — `apps/` có ba tiến trình (`api`, `public-keys`, `unseal-worker`);
  `listOrganizations` được gọi ở `apps/api/src/composition.ts` — ~~`:96`~~ **[S1.79]** neo theo TÊN (khoá `listOrganizations:` của `services`), không theo số dòng: số ấy đã trôi tới 118. Khoản nợ 7.
  **[S1.74]** ~~ba tiến trình~~ **BỐN** — `apps/mcp` ra đời cùng ADR-038: máy chủ MCP chỉ-đọc, nói HTTP
  với `apps/api`, `dependencies` rỗng. Nó là app đầu tiên mang một BỀ MẶT AGENT, và hai khoản nợ của nó
  (141 chứng chỉ, 142 dấu vết kiểm toán) mở cùng ngày.
...
- ~~**Ma trận báo 24/47, không phải 47/47**~~ **[S1.21] con số ấy đã thiu; sổ đăng ký nay có
  ~~**54** bất biến (34 nghiệp vụ + 20 hàng rào)~~ **[S1.78 / lượt soi ngang 72] con số ấy CŨNG đã
  thiu — 56 (34 + 22) ở HEAD** và tỉ lệ phủ nằm ở chính `evidence/INV-matrix.md`.
  ...
  - ~~**23 hàng trống**, mỗi hàng một lý do được ghim trong mã (`MA_DUOC_PHEP_CHUA_PHU`). Nhóm A,
    ...
    **[S1.78] KHÔNG còn hàng trống nào** — các vòng S1 đã lấp hết. Lập luận *"chủ ngữ chưa tồn tại"*
    đúng lúc nó được viết và nay không còn chủ ngữ nào chưa tồn tại.
  - ~~**5 mã mang cờ "phạm vi hẹp hơn mệnh đề"** (D1, D5, E3, F1, G1)~~ **[S1.78] con số và danh sách
    ấy đã thiu; đọc thẳng §4 của ma trận** — ô ✅ **không** có nghĩa mệnh đề đã được phủ trọn vẹn.
  - **[S1.78 / lượt soi ngang 72] Ba con số vừa gạch là ba bản sao, và đoạn này TỰ CẤM chép chúng**
    ("Cố ý KHÔNG chép một con số mới vào đây: chép là tạo bản sao thứ hai"). Chúng vẫn được chép, và
    cả ba cùng thiu — đúng lớp lỗi mà khoản **72** nêu. Nên bản sửa này KHÔNG viết ba con số mới:
    nguồn duy nhất là `evidence/INV-matrix.md`.
...
~~**22 khoản**~~ ~~**[S1.21] 61 khoản, trong đó 14 còn mở**~~ ~~**[S1.23] 63 khoản, trong đó 14 còn mở**~~ ~~**[S1.24] 65 khoản, trong đó 14 còn mở**~~ ~~**[S1.25] 67 khoản, trong đó 13 còn mở**~~ ~~**[S1.26] 69 khoản, trong đó 13 còn mở**~~ ~~**[S1.27] 71 khoản, trong đó 14 còn mở**~~ ~~**[S1.28] 72 khoản, trong đó 14 còn mở**~~ ~~**[S1.29] 73 khoản, trong đó 13 còn mở**~~ ~~**[S1.29] 74 khoản, trong đó 14 còn mở**~~ ~~**[S1.30] 75 khoản, trong đó 14 còn mở**~~ ~~**[S1.31] 76 khoản, trong đó 13 còn mở**~~ ~~**[S1.32] 78 khoản, trong đó 14 còn mở**~~ ~~**[S1.33] 78 khoản, trong đó 13 còn mở**~~ ~~**[S1.34] 78 khoản, trong đó 12 còn mở**~~ ~~**[S1.35] 82 khoản, trong đó 16 còn mở**~~ ~~**[S1.36] 82 khoản, trong đó 15 còn mở**~~ ~~**[S1.37] 83 khoản, trong đó 15 còn mở**~~ ~~**[S1.38] 84 khoản, trong đó 16 còn mở**~~ ~~**[S1.39] 84 khoản, trong đó 15 còn mở**~~ ~~**[S1.40] 85 khoản, trong đó 14 còn mở**~~ ~~**[S1.41] 86 khoản, trong đó 14 còn mở**~~ ~~**[S1.42] 89 khoản, trong đó 17 còn mở**~~ ~~**[S1.43] 89 khoản, trong đó 16 còn mở**~~ ~~**[S1.44] 90 khoản, trong đó 16 còn mở**~~ ~~**[S1.45] 90 khoản, trong đó 15 còn mở**~~ ~~**[S1.46] 91 khoản, trong đó 15 còn mở**~~ ~~**[S1.47] 92 khoản, trong đó 15 còn mở**~~ ~~**[S1.48] 93 khoản, trong đó 16 còn mở**~~ ~~**[S1.49] 93 khoản, trong đó 15 còn mở**~~ ~~**[S1.50] 94 khoản, trong đó 15 còn mở**~~ ~~**[S1.51] 95 khoản, trong đó 15 còn mở**~~ ~~**[S1.52] 96 khoản, trong đó 15 còn mở**~~ ~~**[S1.53] 98 khoản, trong đó 16 còn mở**~~ ~~**[S1.54] 99 khoản, trong đó 16 còn mở**~~ ~~**[S1.55] 99 khoản, trong đó 15 còn mở**~~ ~~**[S1.56] 101 khoản, trong đó 16 còn mở**~~ ~~**[S1.57] 101 khoản, trong đó 15 còn mở**~~ ~~**[S1.58] 102 khoản, trong đó 15 còn mở**~~ ~~**[S1.59] 104 khoản, trong đó 16 còn mở**~~ ~~**[S1.60] 104 khoản, trong đó 15 còn mở**~~ ~~**[S1.61] 105 khoản, trong đó 15 còn mở**~~ ~~**[S1.62] 105 khoản, trong đó 14 còn mở**~~ ~~**[S1.63] 106 khoản, trong đó 14 còn mở**~~ ~~**[S1.64] 107 khoản, trong đó 14 còn mở**~~ ~~**[S1.65] 108 khoản, trong đó 14 còn mở**~~ ~~**[S1.66] 118 khoản, trong đó 24 còn mở**~~ ~~**[S1.67] 119 khoản, trong đó 24 còn mở**~~ ~~**[S1.68] 121 khoản, trong đó 25 còn mở**~~ ~~**[S1.69] 124 khoản, trong đó 27 còn mở**~~ ~~**[S1.70] 125 khoản, trong đó 27 còn mở**~~ ~~**[S1.71] 128 khoản, trong đó 29 còn mở**~~ ~~**[S1.72] 138 khoản, trong đó 38 còn mở**~~ ~~**[S1.73] 140 khoản, trong đó 39 còn mở**~~ ~~**[S1.74] 142 khoản, trong đó 41 còn mở**~~ ~~**[S1.75] 143 khoản, trong đó 41 còn mở**~~ ~~**[S1.76] 146 khoản, trong đó 43 còn mở**~~ ~~**[S1.77] 147 khoản, trong đó 43 còn mở**~~ ~~**[S1.78] 149 khoản, trong đó 45 còn mở**~~ ~~**[S1.79] 152 khoản, trong đó 48 còn mở**~~ ~~**[S1.80] 153 khoản, trong đó 49 còn mở**~~ ~~**[S1.81] 161 khoản, trong đó 57 còn mở**~~ ~~**[S1.82] 167 khoản, trong đó 62 còn mở**~~ ~~**[S1.83] 175 khoản, trong đó 70 còn mở**~~ ~~**[S1.84] 176 khoản, trong đó 69 còn mở**~~ ~~**[S1.85] 177 khoản, trong đó 68 còn mở**~~ ~~**[S1.86] 178 khoản, trong đó 68 còn mở**~~ ~~**[S1.87] 189 khoản, trong đó 79 còn mở**~~ ~~**[S1.90] 196 khoản, trong đó 83 còn mở**~~ ~~**[S1.91] 196 khoản, trong đó 81 còn mở**~~ ~~**[S1.92] 198 khoản, trong đó 82 còn mở**~~ ~~**[S1.93] 202 khoản, trong đó 85 còn mở**~~ ~~**[S1.94] 203 khoản, trong đó 84 còn mở**~~ ~~**[S1.96] 203 khoản, trong đó 82 còn mở**~~ ~~**[S1.97] 204 khoản, trong đó 83 còn mở**~~ ~~**[S1.98] 204 khoản, trong đó 81 còn mở**~~ ~~**[S1.101] 218 khoản, trong đó 85 còn mở**~~ ~~**[S1.103] 218 khoản, trong đó 84 còn mở**~~ ~~**[S1.107] 225 khoản, trong đó 89 còn mở**~~ ~~**[S1.108] 227 khoản, trong đó 91 còn mở**~~ ~~**[S1.109] 230 khoản, trong đó 93 còn mở**~~ ~~**[S1.110] 233 khoản, trong đó 95 còn mở**~~ ~~**[S1.111] 234 khoản, trong đó 96 còn mở**~~ ~~**[S1.112] 235 khoản, trong đó 96 còn mở**~~ ~~**[S1.116] 239 khoản, trong đó 97 còn mở**~~ ~~**[S1.118] 239 khoản, trong đó 96 còn mở**~~ ~~**[S1.118] 239 khoản, trong đó 95 còn mở**~~ ~~**[S1.118] 239 khoản, trong đó 94 còn mở**~~ ~~**[S1.118] 239 khoản, trong đó 93 còn mở**~~ ~~**[S1.129] 239 khoản, trong đó 92 còn mở**~~ ~~**[S1.132] 239 khoản, trong đó 91 còn mở**~~ **[S1.140] 242 khoản, trong đó 93 còn mở** — đầy đủ ở `docs/STATE.md` §*Nợ kỹ
...
3. **Không lớp máy nào cưỡng chế quy ước `OPERATOR(pg_catalog.=)`** — chú thích + test là tất
   ...
   **[S1.79] Khoản 8 ĐÓNG từ S1.22** — `[INV-H21]` + ADR-030 dựng đúng cái lớp máy mà câu trên nói là
   không có; nay nó là một cổng trong `tests/architecture`, không còn là *"chú thích + test"*.
...
và "lời khai nào rộng hơn mã"; hai lượt đầu đều tìm ra thứ bảy lượt dọc trước đó không thấy. ~~Lượt kế: sau S1.47.~~ **[S1.48]** Lượt 40 chạy sau S1.47 (S1.43–S1.47 đổi hardening NĂM lần liên tiếp — điều kiện "ba lần" đã thoả từ S1.45; năm vòng cố ý gộp vì cùng chủ đề danh tính/GUC, lịch thắng điều kiện — 40b #16); lượt 40 tìm ra một hồi quy I1 do chính S1.47 tạo. ~~Lượt kế: sau ba vòng đổi hardening nữa, hay chậm nhất S1.52.~~ **[S1.66 / lượt soi ngang 59c NẶNG-4] Lỡ nhịp, nói ra:** hardening đổi ở mỗi vòng từ S1.50 tới S1.58, mốc "chậm nhất S1.52" qua mà không lượt ngang nào chạy, và không biên bản nào ghi lỡ nhịp tới S1.65. Lượt 59 chạy ở S1.66 trên S1.49–S1.65 và tìm ra một hồi quy do S1.59 tạo (bộ dọn nền không listener `'error'`) cùng một lời khai gãy của sổ nợ (con trỏ biên bản 66–80 — con trỏ cũ nhất sống mười lăm vòng, S1.51–S1.65). ~~Lượt kế: sau ba vòng đổi hardening, hay chậm nhất S1.71;~~ vòng nào chạm mốc mà không chạy lượt ngang thì biên bản của vòng ấy ghi lỡ nhịp kèm lý do. **[S1.71] Lỡ nhịp, nói ra:** S1.71 chạm mốc "chậm nhất S1.71" mà không chạy lượt ngang — hardening đổi hai lần kể từ lượt 59 (S1.66, S1.71), điều kiện ba lần chưa thoả; lý do ở `evidence/security-reviews.md` §S1.71. ~~Lượt kế: chậm nhất S1.72 — vòng khoản 121 chạy lượt ngang trên S1.66–S1.71 cùng lượt soi dọc của nó, hay ghi lỡ nhịp kèm lý do.~~ **[S1.72]** Lượt 66 chạy ở S1.72 trên S1.66–S1.71 (PR #65–#70, master `298cd4e`) cùng lượt soi dọc 67 của khoản 121 — đúng mốc; S1.72 không đổi hardening. ~~Lượt kế: sau ba vòng đổi hardening, hay chậm nhất S1.77; vòng nào chạm mốc mà không chạy lượt ngang thì biên bản của vòng ấy ghi lỡ nhịp kèm lý do.~~ **[S1.78] LỠ NHỊP Ở S1.77, nói ra — và lý do không đẹp.** S1.77 chạm đúng mốc *“chậm nhất S1.77”* mà không chạy lượt ngang. Lý do **KHÔNG** phải điều kiện chưa thoả: nó là vòng ấy **không kiểm mốc ở đầu vòng**, và chỉ phát hiện ra khi đã merge xong. Khác hai lần lỡ nhịp trước — S1.66 (mốc S1.52 qua mà không ai ghi tới S1.65) và S1.71 (chạm mốc, điều kiện ba vòng chưa thoả nên ghi lý do) — lần này mốc đã chạm VÀ không có lý do nào ngoài việc quên. Đo vế còn lại cho đủ: `hardening.always.sql` **không đổi lần nào** kể từ lượt 66 (`git rev-list --count 70075e8..4e3118f --first-parent -- db/migrations/hardening.always.sql` = 0), nên vế *“ba vòng đổi hardening”* chưa thoả; vế lịch thì đã chạm, và hai vế nối bằng **hay**. **[S1.78] Lượt 72 chạy ở S1.78** trên S1.72–S1.77 (PR #71, #73–#77; master `70075e8` → `4e3118f`) — **muộn một vòng**, chủ dự án chọn chạy ngay thay vì dời. Cửa sổ này có một tính chất chưa cửa sổ nào trước có: **ba vòng trong sáu vòng chạy SONG SONG trên các worktree khác nhau rồi gộp lại**, và biên bản §S1.76 tự ghi rằng một lần gộp như thế đã để lọt hai lỗ ở đúng chỗ Git không báo gì — nên lượt 72 lấy chỗ giáp ranh ấy làm một góc soi riêng. ~~Lượt kế: sau ba vòng đổi hardening, hay chậm nhất **S1.83**;~~ vòng nào chạm mốc mà không chạy lượt ngang thì biên bản của vòng ấy ghi lỡ nhịp kèm lý do — **và vòng nào cũng phải kiểm mốc này ở ĐẦU vòng, không phải sau khi merge.** **[S1.79]** Vòng S1.79 ĐÃ kiểm mốc ở đầu vòng — S1.79 chưa chạm S1.83, nên không lượt ngang nào chạy và không có lỡ nhịp. **[S1.83] Lượt 73 chạy ĐÚNG MỐC** trên S1.78–S1.82 (PR #78–#82; master `4e3118f` → `1dfc7e3`), và đo cả hai vế của điều kiện thay vì chỉ nói "tới hạn": vế **lịch** ĐÃ chạm (vòng này là S1.83); vế *"ba vòng đổi hardening"* **CHƯA thoả** — `git rev-list --count 4e3118f..1dfc7e3 --first-parent -- db/migrations/hardening.always.sql` = **1** (đổi duy nhất ở S1.82). Hai vế nối bằng **hay**, nên một vế đủ. Ghi ra vì một lời khai *"cả hai vế đều nghiêng về chạy ngay"* đã được viết ở sơ đồ trạng thái trước khi đo, và phép đo bác nó. Lượt 73: 66 agent, 6 góc, 30 phát hiện qua thẩm tra đối kháng (27 sống, 3 bị bác); một CAO là **hồi quy do chính lượt 72 tạo ra**. Lượt kế: sau ba vòng đổi hardening, hay chậm nhất **S1.89**; vòng nào chạm mốc mà không chạy lượt ngang thì biên bản của vòng ấy ghi lỡ nhịp kèm lý do — và vòng nào cũng phải kiểm mốc này ở ĐẦU vòng, không phải sau khi merge. **[S1.84]** Vòng S1.84 ĐÃ kiểm mốc ở đầu vòng — chưa chạm S1.89, hardening không đổi. **[S1.85]** Vòng S1.85 ĐÃ kiểm mốc ở đầu vòng và đo CẢ HAI vế thay vì chỉ đọc lịch: vế lịch chưa chạm (S1.85 < S1.89), và `git rev-list --count 1dfc7e3..HEAD --first-parent -- db/migrations/hardening.always.sql` = **1**, chưa tới ba. Không lượt ngang nào chạy, không lỡ nhịp; S1.85 cũng không đổi hardening. Lượt quét lời khai thiu của S1.79 **KHÔNG tính là lượt ngang**: nó chỉ soi một lớp lỗi (lời khai bị HEAD làm cho sai), không soi hồi quy, không soi lớp cưỡng chế, không soi test mất răng. **[S1.87] LƯỢT 74 CHẠY SỚM — CHỦ DỰ ÁN CHỌN, VÀ CẢ HAI VẾ ĐỀU ĐƯỢC ĐO CHỨ KHÔNG ĐOÁN.** Vế lịch **chưa** chạm (S1.87 < S1.89); vế hardening `git rev-list --count 1dfc7e3..HEAD --first-parent -- db/migrations/hardening.always.sql` = **2**, chưa tới ba. Tức **không vế nào thoả** — đây không phải một lượt đúng mốc mà là một lượt chạy TRƯỚC mốc, theo chỉ thị của chủ dự án sau khi S1.86 vừa đổi hardening lần thứ hai. Lý do đứng vững sau lượt: hai phát hiện CAO đều nằm TRONG cửa sổ S1.83–S1.86 và một trong hai nằm trong ADR-042 — chấp nhận chưa tới một ngày trước đó. Lượt 74: 6 góc, góc *hồi quy xuyên vòng* cho kết quả **ÂM**, tổng **2 CAO · ~11 TRUNG · ~8 THẤP**; mọi phát hiện nặng được tự đo lại bằng tay, không cái nào bị bác. Chủ dự án chọn **tách hai vòng**: S1.87 sửa LỜI KHAI (không đổi hành vi), vòng sau vá CỔNG. Mốc kế giữ nguyên: sau ba vòng đổi hardening, hay chậm nhất **S1.89**. **[S1.88] LỠ NHỊP, nói ra — và lần này vế hardening đã THOẢ.** Vòng S1.88 kiểm mốc và đo cả hai vế: vế lịch chưa chạm (S1.88 < S1.89), nhưng `git rev-list --count 1dfc7e3..HEAD --first-parent -- db/migrations/hardening.always.sql` = **3** — đủ ba, vì chính S1.87 đổi hardening lần thứ ba. Hai vế nối bằng **hay** nên mốc ĐÃ CHẠM, và S1.88 không chạy lượt ngang 75. Lý do đo được: S1.88 đổi **0 dòng mã sản phẩm, 0 SQL, 0 test** (chỉ tài liệu và ADR-043), nên lượt ngang sẽ soi đúng cây mà lượt 74 vừa soi; và một vòng khác đang chạy song song trên `khoan-179-ve-cong-log-tu-choi` với 11 tệp mã/test đang đổi. **Mốc mới: ngay sau khi vòng ấy merge, chậm nhất S1.90** — và theo ADR-043, lượt 75 lấy rổ A làm một góc soi riêng. **[S1.90] LỠ NHỊP, nói ra — mốc đã chạm CẢ HAI VẾ và vòng này vẫn không chạy lượt ngang.** Kiểm ở ĐẦU vòng và đo cả hai: vế lịch **chạm** (vòng này là S1.90); vế hardening `git rev-list --count 1dfc7e3..origin/master --first-parent -- db/migrations/hardening.always.sql` = **3**, đủ ba. Nhánh `khoan-179-ve-cong-log-tu-choi` chưa đẩy lên origin và không PR nào mở, nên vế *ngay sau khi vòng ấy merge* không còn giữ được nữa. Lý do KHÔNG chạy, đo được chứ không phải quên: ➀ ADR-043 buộc lượt 75 lấy **rổ A** làm một góc soi riêng, mà rổ A vừa đổi trong chính vòng này (14 → 15, cộng ba khoản rổ B mới) — cho lượt 75 soi một sổ nợ chụp ở `1a1a060` là phí một lượt sáu góc; ➁ lượt soi ngang là việc chủ dự án gọi, không phải việc vòng tự gọi. **Mốc mới: chậm nhất S1.92**, và vòng nào chạm mốc mà không chạy thì vẫn ghi lỡ nhịp kèm lý do, kiểm ở ĐẦU vòng. **[S1.91]** Vòng S1.91 ĐÃ kiểm mốc ở đầu vòng và đo cả hai vế: vế lịch **chưa chạm** (S1.91 < S1.92); vế hardening `git rev-list --count 1dfc7e3..origin/master --first-parent -- db/migrations/hardening.always.sql` = **3** — vẫn đủ ba, và vẫn là ba lần đổi mà S1.90 đã đo rồi hoãn, không phải ba lần mới. Không lượt ngang nào chạy; S1.91 cũng không chạm `hardening.always.sql`. Mốc giữ nguyên: chậm nhất **S1.92**. **[S1.92] LỠ NHỊP, nói ra — mốc “chậm nhất S1.92” ĐÃ CHẠM và vòng này vẫn không chạy lượt ngang.** Kiểm ở ĐẦU vòng và đo cả hai vế: vế lịch **chạm** (vòng này là S1.92); vế hardening `git rev-list --count 1dfc7e3..origin/master --first-parent -- db/migrations/hardening.always.sql` = **3**, đủ ba — vẫn là ba lần đổi mà S1.90 đã đo rồi hoãn. Lý do KHÔNG chạy, đo được chứ không phải quên: vòng này sinh ra GIỮA phiên, từ một lượt ĐI THỬ của chủ dự án ngay sau khi S1.91 merge, và chủ dự án chọn vá chặng đánh thức ngay (khoản 197) thay vì mở một lượt sáu góc; lượt soi ngang là việc chủ dự án gọi, không phải việc vòng tự gọi (ADR-043). **Mốc mới: chậm nhất S1.93**, và ADR-043 vẫn buộc lượt 75 lấy rổ A làm một góc soi riêng — nay đọc thêm bài học của vòng này: một khoản **rổ B** cũng có thể đang chặn pilot (khoản 156). **[S1.93] LƯỢT 75 CHẠY — ĐÚNG MỐC, và cửa sổ là năm vòng S1.88–S1.92** (`1a1a060..c3c73ad`, PR #88–#92; 13 commit, 58 tệp, +4345/−57). Sáu góc, trong đó HAI góc bắt buộc: rổ A (ADR-043) và một góc mới — *rổ B xếp nhầm* — sinh ra từ việc khoản 156 nằm ở rổ B suốt mười một vòng trong khi nó chặn đúng kịch bản pilot. Kết quả: **1 CAO** (khoản 199 — một người khoá được cả hai người duyệt, tái lập trên ngăn xếp thật trong 3 giây, VÁ ngay trong vòng này), 2 khoản rổ B lên rổ A (103, 196), 3 khoản mới ghi phần không vá (200, 201, 202), và một chùm lời khai thiu đã sửa. Góc 1 KHÔNG âm. ~~**Mốc kế: sau ba vòng đổi hardening, hay chậm nhất S1.99**~~; vòng nào chạm mốc mà không chạy thì ghi lỡ nhịp kèm lý do, kiểm ở ĐẦU vòng. **[S1.99] LƯỢT 76 CHẠY — ĐÚNG MỐC, và cả hai vế được ĐO chứ không đoán.** Vế **lịch** CHẠM (vòng này là S1.99); vế hardening `git rev-list --count c3c73ad..81dae1b --first-parent -- db/migrations/hardening.always.sql` = **1**, chưa tới ba. Hai vế nối bằng **hay**, nên một vế đủ — ghi ra vì lượt 73 đã có tiền lệ một lời khai *“cả hai vế đều nghiêng về chạy”* bị chính phép đo bác. Cửa sổ: sáu vòng S1.93–S1.98 (`c3c73ad..81dae1b`, PR #93–#98; 6 commit, 43 tệp, +2166/−67). Sáu góc, **33 phát hiện**; mọi phát hiện nặng được tự đo lại bằng tay trước khi đề xuất vá, theo bài học đã ghi rằng thẩm tra đối kháng của lượt 75 bác quá ít (3/57). Kết quả: **4 CAO** — ba trong bốn nằm ở `apps/web/trang/` (1 158 dòng, 0 test, không tsc, không cổng kiến trúc), và **hai trong bốn là khiếm khuyết của chính vòng S1.98** (khoản 205 và lời khai sai của hàng 204). Chủ dự án chốt vá nhóm `apps/web` trước; năm phát hiện còn lại ghi thành khoản 208–212. **Điều đáng ghi nhất về lượt này:** góc *lời khai rộng hơn mã* đọc kỹ mọi mục *Ranh giới nói ra* của §S1.94–§S1.98 và KHÔNG tìm được câu nào rộng hơn phép đo — thứ trôi ở kho này là các CON SỐ viết cứng, không phải các mục ranh giới. ~~**Mốc kế: sau ba vòng đổi hardening, hay chậm nhất S1.105**~~ **[S1.105] LỠ NHỊP, nói ra:** S1.105 chạm đúng mốc mà không chạy lượt ngang — vòng ấy là S2.3, một vòng mã, và chủ dự án chọn đóng nó trước; mốc mới khi ấy là **chậm nhất S1.107**. **[S1.106]** Vòng S1.106 kiểm mốc ở ĐẦU vòng: chưa chạm S1.107, không lượt ngang nào chạy, không lỡ nhịp. **[S1.107] LƯỢT 77 CHẠY — ĐÚNG MỐC, và cả hai vế được ĐO chứ không đoán.** Vế **lịch** CHẠM (vòng này là S1.107); vế hardening `git rev-list --count 81dae1b..dc2b560 --first-parent -- db/migrations/hardening.always.sql` = **2**, chưa tới ba. Hai vế nối bằng **hay**, nên một vế đủ. Cửa sổ: TÁM vòng S1.99–S1.106 (`81dae1b..dc2b560`, PR #99–#106; 22 commit, 55 tệp, +6467/−151) — S1.99 nằm TRONG cửa sổ vì lượt 76 chạy Ở S1.99 và soi S1.93–S1.98, nên bản vá của chính S1.99 chưa ai soi. Sáu góc, và góc sinh ra nhiều nhất là một góc MỚI: *cổng XANH vì phạm vi của nó*. Kết quả: **2 CAO** — `EVALUATING` không có cạnh ra nào trong khi S1.106 vừa mở cửa vào nó ra HTTP, và KHÔNG đường sản xuất nào ghi được `eval_components` nên route chấm thầu của S1.106 luôn 422 ngoài cụm test — cộng ba TRUNG BÌNH; cả năm VÁ TRONG VÒNG. Sáu phép đo ÂM được ghi để vòng sau không làm lại. **Mốc kế: sau ba vòng đổi hardening, hay chậm nhất S1.113**; vòng nào chạm mốc mà không chạy thì ghi lỡ nhịp kèm lý do, kiểm ở ĐẦU vòng. **[S1.108]** Vòng S1.108 ĐÃ kiểm mốc ở ĐẦU vòng và đo CẢ HAI vế chứ không đọc lịch: vế **lịch** CHƯA chạm (S1.108 < S1.113); vế **hardening** `git rev-list --count 305a090..origin/master --first-parent -- db/migrations/hardening.always.sql` = **0** — chính vòng này là lần đổi THỨ NHẤT kể từ lượt 77, nên sau khi merge con số là **1**, chưa tới ba. Không lượt ngang nào chạy, không lỡ nhịp. Nhưng vòng này CÓ chạy một lượt soi **HÌNH DẠNG** trước dòng mã đầu — khuôn S1.75/S1.101, bắt buộc với vòng chạm lõi niêm phong — và nó tìm ra **ba** thứ làm đổi thiết kế trước khi có mã: C1 chặn mọi lần nộp BAFO, C3 cùng vế 3 của cổng bốn vế ghim cứng `'CLOSED'`, và spec §4.3 không có trạng thái nào nghĩa là *phong bì BAFO đã mở*. Lượt ấy KHÔNG tính là lượt ngang: nó soi một hình dạng CHƯA cài, không soi hồi quy xuyên vòng. Mốc giữ nguyên: **chậm nhất S1.113**. **[S1.109] LỠ NHỊP, nói ra — và MỘT CON SỐ CỦA CHÍNH MỤC NÀY ĐÃ SAI MỘT NHỊP.** Vòng S1.109 kiểm mốc ở ĐẦU vòng và đo cả hai vế. Vế **lịch** CHƯA chạm (S1.109 < S1.113). Vế **hardening**: câu đo của S1.108 dùng gốc `305a090..origin/master` và ra **0**, rồi kết luận *“chính vòng này là lần đổi THỨ NHẤT kể từ lượt 77”*. Gốc ấy SAI một nhịp: `305a090` là merge của S1.107, còn cửa sổ lượt 77 **kết ở `dc2b560`** (merge của S1.106) — nên lần đổi hardening của chính S1.107 chưa lượt ngang nào soi. Đo lại bằng gốc đúng: `git rev-list --count dc2b560..origin/master --first-parent -- db/migrations/hardening.always.sql` = **2** (S1.107 `305a090`, S1.108 `a0e4cf2`), nên S1.109 là lần đổi **THỨ BA** và vế hardening **ĐÃ THOẢ** — hai vế nối bằng **hay**, tức mốc CHẠM. **Lý do KHÔNG chạy, đo được chứ không phải quên:** S1.107 · S1.108 · S1.109 là ba vòng của CÙNG một chủ đề (cạnh trạng thái RFQ rồi vòng BAFO), và hai trong ba đã có một lượt soi HÌNH DẠNG chạy trước dòng mã đầu — lượt 78 mở ngay bây giờ sẽ soi đúng cây mà hai lượt ấy vừa soi, với cùng một cặp mắt. Cộng vế thủ tục: lượt soi ngang là việc chủ dự án gọi, không phải việc vòng tự gọi (ADR-043). **Mốc mới: ngay sau S1.109, chậm nhất S1.111** — sớm hơn mốc cũ đúng vì vế hardening đã thoả, và cửa sổ của nó là S1.107–S1.109 (`dc2b560..`), KHÔNG phải S1.108 trở đi. **[S1.110] LỠ NHỊP LẦN THỨ HAI LIÊN TIẾP, và lần này cả HAI vế đều đã thoả — nói ra kèm con số.** Vòng S1.110 kiểm mốc ở ĐẦU vòng. Vế **lịch**: mốc là *chậm nhất S1.111*, nên S1.110 CHƯA chạm theo lịch — nhưng nó là vòng cuối trước mốc. Vế **hardening**, đo lại bằng gốc ĐÚNG mà S1.109 vừa sửa: `git rev-list --count dc2b560..origin/master --first-parent -- db/migrations/hardening.always.sql` = **3** (S1.107 `305a090`, S1.108 `a0e4cf2`, S1.109 `cb796ec`) — và chính vòng này là lần đổi **THỨ TƯ**, vì `061` re-ghim `rfq_kiem_chuyen_trang_thai` cộng ba mục ghim MỚI. Vế hardening vì thế đã thoả từ S1.109 và nay vượt ngưỡng. **Lý do KHÔNG chạy, và nó là một lý do YẾU HƠN lý do của S1.109 — ghi ra để không ai đọc thành một thông lệ:** chủ dự án chọn *chạy trọn S2 trước* (2026-09-22), và S2.6 là hạng mục áp cuối; lượt 78 chạy ngay bây giờ sẽ soi một cây mà S2.7 còn đang đổi. S1.109 lỡ nhịp vì một lập luận về NỘI DUNG (ba vòng cùng chủ đề, hai đã có lượt soi hình dạng); S1.110 lỡ nhịp vì một lập luận về LỊCH, và lập luận lịch là thứ mục này tồn tại để chống. **Hệ quả: mốc KHÔNG dời thêm.** Lượt 78 chạy **chậm nhất S1.111**, cửa sổ `dc2b560..` — nay bốn vòng (S1.107–S1.110) và bốn lần đổi hardening, tức lượt ngang thưa nhất kể từ lượt 40. **[S1.111] LỠ NHỊP LẦN THỨ BA, và mốc rơi ĐÚNG vào vòng này.** Mốc là *chậm nhất S1.111*, nên vế LỊCH nay đã chạm chứ không còn sắp chạm; vế hardening thì vượt ngưỡng từ S1.110. Vòng này là một vòng ĐỔI SỐ tài liệu — không một dòng mã — nên nó KHÔNG có bề mặt để một lượt soi ngang soi, và chạy lượt 78 ở đây sẽ cho một lượt soi trên đúng cây mà S1.110 vừa soi. **Ba lần liên tiếp là một nhịp đã HỎNG, không còn là ba lần hoãn** — và ba lý do thì mỗi lần một yếu hơn: S1.109 lỡ vì NỘI DUNG (ba vòng cùng chủ đề), S1.110 vì LỊCH, S1.111 vì KHÔNG CÓ MÃ. Mục này tồn tại để chống đúng chuỗi lập luận ấy. **Không tự dời mốc thêm một vòng nữa:** lượt 78 là việc chủ dự án gọi (ADR-043), và ba lần lỡ liên tiếp là thứ phải trình chứ không phải thứ tự gia hạn. Cửa sổ vẫn `dc2b560..`, nay là năm vòng. **[S1.113] LƯỢT 78 CHẠY — chủ dự án gọi, sau BA lần lỡ nhịp liên tiếp, và cả hai vế đều được ĐO.** Vế **lịch**: mốc là *chậm nhất S1.111*, vòng này là S1.113 ⇒ CHẠM và **quá hạn hai vòng**. Vế **hardening**: `git rev-list --count dc2b560..origin/master --first-parent -- db/migrations/hardening.always.sql` = **4**, vượt ngưỡng ba. **Cả hai** vế thoả — và đó là hệ quả của ba lần hoãn chứ không phải một thành tích: mỗi lần hoãn lại đẩy thêm một lần đổi hardening vào cùng một cửa sổ. Cửa sổ: NĂM vòng S1.107–S1.111 (`dc2b560..9920b09`, PR #107–#111; 15 commit, 50 tệp, +8957/−184), cộng S1.112 gộp vào trước khi mở PR; S1.107 nằm TRONG cửa sổ vì lượt 77 chạy *ở* S1.107 và soi S1.99–S1.106. Sáu góc chạy TUẦN TỰ, không fan-out, mọi phát hiện tự đo lại bằng tay — cùng khuôn lượt 77. Kết quả: **2 CAO** đều nằm ở chính bộ máy ĐÍCH chứ không trong mã sản phẩm (ô *mảnh 2* của bảng bốn mảnh sai từ S1.110; vế ⒞ của ADR-043 chưa chạy lần nào trong 12 vòng và không cổng nào đọc nó), **1 TRUNG BÌNH** ba cái sàn chống-mù trôi 21–32%, **1 đề xuất CHƯA ÁP** chờ chủ dự án (rổ của khoản 233 đọc thiếu vế ⒝ của ADR-043), và **sáu phép đo ÂM** ghi ra để vòng sau không làm lại — trong đó góc *hồi quy xuyên vòng* ÂM, khác lượt 73 vốn tìm ra một CAO do chính lượt 72 tạo. **Mốc kế: sau ba vòng đổi hardening, hay chậm nhất S1.119**; vòng nào chạm mốc mà không chạy thì ghi lỡ nhịp kèm lý do, kiểm ở ĐẦU vòng. **Và một thứ vòng này ĐÃ chạy, không thay được lượt ngang nhưng phải đọc kèm:** lượt soi HÌNH DẠNG trước dòng mã đầu tìm ra ba CAO (lỗ J3 sau điều phối lại, `AWARDED` chạm SÁU ràng buộc chứ không bốn, `AWARDED` sẽ là trạng thái hút thứ ba nếu §8.3 không chốt), và bốn trong số đó thành quyết định của chủ dự án trước khi có mã.

---

**[2026-09-19 / S1.88 / ADR-043] VIỆC KẾ TIẾP — ĐỌC BỐN VIỆC NÀY THAY CHO MƯỜI MỘT MỤC Ở TRÊN.**
Mười một mục trên là việc của S0 và của đầu S1; chúng đã xong hoặc đã có chỗ treo. **S1 đóng ngày
2026-09-19** (bảng đối chiếu đo trên `1a1a060` ở `docs/STATE.md` §*Cột mốc hiện tại*), và đích mới là
kịch bản phát biểu bằng hành vi người mua ở `docs/PRODUCT.md` §11. Bốn việc, đúng thứ tự này:

1. **Rổ A — ~~mười bốn~~ ~~**[S1.95] mười**~~ ~~**[S1.96] tám**~~ ~~**[S1.98] bảy**~~ **[S1.99] SÁU** khoản chặn pilot.** *(Con số “mười bốn” sống từ S1.88 qua SÁU vòng trong khi dòng ngay dưới được viết lại ở mỗi vòng — lượt soi ngang 76 góc 6 đo ra, và nó là hàng đầu của mục “việc tiếp theo”; xem khoản 212.)* Danh sách và lý do từng khoản ở `docs/STATE.md` §*Nợ kỹ thuật*,
   ngay dưới đoạn đếm: ~~15 · 102 · 104 · 105 · 108 · 109 · 125 · 130 · 135 · 154 · 155 · 159 · 160 · 165~~ ~~**[S1.93] 15 · 102 · 103 · 104 · 105 · 108 · 109 · 125 · 130 · 135 · 155 · 159 · 160 · 165 · 196 · 198**~~ ~~**[S1.94] 15 · 102 · 104 · 105 · 108 · 109 · 125 · 130 · 135 · 155 · 159 · 160 · 165 · 196 · 198**~~ ~~**[S1.95] 15 · 102 · 105 · 109 · 125 · 130 · 159 · 165 · 196 · 198**~~ ~~**[S1.96] 15 · 102 · 105 · 109 · 125 · 165 · 196 · 198**~~ ~~**[S1.98] 15 · 102 · 105 · 109 · 165 · 196 · 198**~~ **[S1.99] 15 · 102 · 105 · 109 · 165 · 196** (125 ĐÓNG; 198 ĐÓNG ở S1.99) (154 ĐÓNG ở S1.91; 198 vào ở S1.92; 103 và 196 lên từ rổ B ở S1.93).
   ~~Bốn mươi bảy~~ ~~**[S1.93] Năm mươi mốt**~~ ~~**[S1.94] Năm mươi**~~ ~~**[S1.95] Năm mươi tư**~~ ~~**[S1.97] Năm mươi lăm**~~ ~~**[S1.98] Năm mươi tư**~~ ~~**[S1.99] Năm mươi tám**~~ ~~**[S1.100] Năm mươi bảy**~~ **[S1.101] Năm mươi tám** khoản rổ B **đóng băng** tới sau pilot, ~~mười tám~~ ~~**[S1.94] mười chín**~~ ~~**[S1.95] hai mươi**~~ ~~**[S1.99] hai mươi mốt**~~ ~~**[S1.99] hai mươi hai**~~ **[S1.101] hai mươi mốt** khoản rổ C thành *giới hạn đã biết*. Một
...
2. ~~**Spec S2 — Đánh giá, BAFO, Award.** Lát cắt MVP1 duy nhất chưa có spec (`docs/PRODUCT.md` §7), ước
   ...
   **[S1.103] SPEC ĐÃ CÓ — vào kho ở S1.101**, `docs/superpowers/specs/2026-09-21-trustprocure-s2-danh-gia-bafo-award.md`.
   ~~Việc còn lại là **CÀI**: S2.1 xong ở S1.102 (`056`), S2.2–S2.7 chưa có dòng nào, và ADR-051 ghi rằng **J3 chưa
   có lớp nào cưỡng chế** — lớp thật của nó nằm ở S2.6.~~
   **[S1.114 — KHOẢN 238] HAI CÂU VỪA GẠCH LÀ BẢN SAO CỦA HAI CÂU MÀ S1.113 ĐÃ GẠCH Ở `docs/PRODUCT.md` §11 mảnh 2,
   VÀ CHÚNG Ở LẠI ĐÂY THÊM MỘT VÒNG.** Vá điểm, không vá lớp — ở chính vòng đặt tên cho lớp ấy. Đo lại ngày
   2026-09-23: **S2.1 → S2.7 XONG CẢ BẢY** (S1.102 · S1.104 · S1.105 · S1.106 · S1.108+S1.109 · S1.110 · S1.114),
   và **J3 CÓ lớp cưỡng chế** từ S1.110 — `061` giao ba trigger trong đó có `award_kiem_de_xuat`, với đúng một lỗ
   đã khai là khoản **233** (rổ A từ 2026-09-22).
   **Câu cuối của dòng cũ đáng đọc riêng, vì nó không thiu mà bị BÁC:** *“không viết được đúng nếu chưa có việc 4”*
   — việc 4 là khách hàng pilot, và spec được viết khi **vẫn chưa có pilot**. Dự án đã chọn đi trước điều kiện mình
   tự đặt ra; ghi lại ở đây chứ không xoá, vì cái giá của lựa chọn ấy chỉ đo được khi có người mua thật đọc spec.
3. ~~**Lát cắt giao diện tối thiểu.** Hai màn: nhà cung cấp nộp thầu trên điện thoại (đây là nơi
   `crypto.subtle` chạy — USP số 1 sống hay chết ở đây), và người mua mở thầu rồi đọc bảng so sánh. Hôm
   nay kho có **đúng một** tệp `.html` và nó là máy dò ở `tools/do-webcrypto/`.~~
   **[S1.89 / ADR-044] XONG, và đã chạy THẬT đầu tới cuối** — `apps/web` (hai màn + bộ chuyển tiếp),
   `packages/sealed-envelope/src/browser.ts` (cửa trình duyệt), `tools/gieo-demo` (gieo bối cảnh).
   Số đo của lượt chạy ở `docs/STATE.md` §*Cột mốc hiện tại*; cách chạy ở ADR-044 §*Cách chạy*.
   Việc còn lại của mảnh này KHÔNG phải mã: **HTTPS cho điện thoại thật** (cookie phiên khách mang
   `Secure`, nên `http://` ở địa chỉ LAN im lặng thất bại) — hai biến `TRUSTPROCURE_WEB_TLS_*` đã có chỗ.
4. **Khách hàng pilot.** Rủi ro số 1 trong mọi tài liệu từ 2026-08-27, và tới 2026-09-19 **chưa ai chạm**.
   Nó không phải việc của mã nguồn, và nó chặn việc 2.

**Một việc CHƯA quyết, ghi ra để không ai đọc im lặng thành đã quyết:** nhịp lượt soi ngang ở cuối mục này
**giữ nguyên** (~~mốc kế: chậm nhất S1.89~~ ~~**[S1.93] lượt 75 đã chạy; mốc kế: chậm nhất S1.99**~~ ~~**[S1.100] lượt 76 đã chạy Ở S1.99; mốc kế: chậm nhất S1.105**~~ **[S1.105] MỐC ẤY ĐÃ TRƯỢT: S1.105 là vòng này và nó KHÔNG phải lượt soi.** Bốn vòng S1.102–S1.105 đều là vòng cài (S2.1–S2.3) và chủ dự án chọn chạy tiếp thay vì soi. Ghi ra chứ không đẩy mốc lặng lẽ; **mốc mới: chậm nhất S1.107**. Vế hardening của nhịp: S1.105 đổi `hardening.always.sql` — 2 trong 3 (S1.100 là lần đầu) *(dòng này THIU một vòng: S1.99 chạy lượt 76 mà không sửa nó, nên một người đọc ở đầu S1.100 thấy một mốc ĐÃ QUA và không biết lượt gần nhất là lượt nào. Vế hardening của nhịp: S1.100 đổi `hardening.always.sql` — 1 trong 3. Cùng họ khoản 212.)* — xem cuối §11). ADR-043 không đụng tới nó. Câu hỏi *"soi theo lát cắt tính năng
thay vì theo lịch"* là một quyết định riêng, chưa trình, chưa chốt.
...
pnpm test        # T0/T1, không cần Docker
pnpm test:int    # tích hợp, CẦN Docker Desktop đang chạy
pnpm evidence    # sinh lại ma trận + cổng evidence
```

**[S1.66 / lượt soi ngang 59c NHẸ-13]** Khối lệnh trên từng ghi `pnpm test` 346 và `pnpm test:int` 326 — thiu từ lâu (biên bản S1.65 ghi `pnpm test` 50 tệp / 748 test — chưa đếm máy). Số đo của mỗi vòng nằm ở biên bản vòng trong `evidence/security-reviews.md`, không chép ở đây.

**Hai điều dễ vấp:**

- ~~**Checkout mới trên Windows: mọi file `.ts` sẽ là CRLF.** `.gitattributes` chỉ ghim `*.sql`
  và `evidence/INV-matrix.md`.~~ **[S1.66 / lượt soi ngang 59c NHẸ-13]** Từ S1.63 (khoản 10) `.gitattributes` ghim thêm
  `*.ts text eol=lf`: checkout mới cho `.ts` ở LF. Một cây làm việc có sẵn giữ nguyên byte cho tới khi Git ghi lại tệp (S1.63
  ranh giới ⑴) — S1.66 gặp đúng ca ấy: `git ls-files --eol` báo `w/crlf` cho `packages/db/src/migrate.ts`. Tệp không ghim vẫn
  theo `core.autocrlf`. Đây không phải lỗi, nhưng mọi công cụ đo byte phải dùng **công
...
| Tài liệu | Vì sao |
|---|---|
| `docs/STATE.md` | **Đọc đầu tiên.** Sổ trạng thái đầy đủ: điều kiện hoàn thành, điểm chặn, ~~22 khoản nợ~~ ~~**[S1.21] 61 khoản nợ, 14 còn mở**~~ ~~**[S1.27] 71 khoản, trong đó 14 còn mở**~~ ~~**[S1.28] 72 khoản, trong đó 14 còn mở**~~ ~~**[S1.29] 73 khoản, trong đó 13 còn mở**~~ ~~**[S1.29] 74 khoản, trong đó 14 còn mở**~~ ~~**[S1.30] 75 khoản, trong đó 14 còn mở**~~ ~~**[S1.31] 76 khoản, trong đó 13 còn mở**~~ ~~**[S1.32] 78 khoản, trong đó 14 còn mở**~~ ~~**[S1.33] 78 khoản, trong đó 13 còn mở**~~ ~~**[S1.34] 78 khoản, trong đó 12 còn mở**~~ ~~**[S1.35] 82 khoản, trong đó 16 còn mở**~~ ~~**[S1.36] 82 khoản, trong đó 15 còn mở**~~ ~~**[S1.37] 83 khoản, trong đó 15 còn mở**~~ ~~**[S1.38] 84 khoản, trong đó 16 còn mở**~~ ~~**[S1.39] 84 khoản, trong đó 15 còn mở**~~ ~~**[S1.40] 85 khoản, trong đó 14 còn mở**~~ ~~**[S1.41] 86 khoản, trong đó 14 còn mở**~~ ~~**[S1.42] 89 khoản, trong đó 17 còn mở**~~ ~~**[S1.43] 89 khoản, trong đó 16 còn mở**~~ ~~**[S1.44] 90 khoản, trong đó 16 còn mở**~~ ~~**[S1.45] 90 khoản, trong đó 15 còn mở**~~ ~~**[S1.46] 91 khoản, trong đó 15 còn mở**~~ ~~**[S1.47] 92 khoản, trong đó 15 còn mở**~~ ~~**[S1.48] 93 khoản, trong đó 16 còn mở**~~ ~~**[S1.49] 93 khoản, trong đó 15 còn mở**~~ ~~**[S1.50] 94 khoản, trong đó 15 còn mở**~~ ~~**[S1.51] 95 khoản, trong đó 15 còn mở**~~ ~~**[S1.52] 96 khoản, trong đó 15 còn mở**~~ ~~**[S1.53] 98 khoản, trong đó 16 còn mở**~~ ~~**[S1.54] 99 khoản, trong đó 16 còn mở**~~ ~~**[S1.55] 99 khoản, trong đó 15 còn mở**~~ ~~**[S1.56] 101 khoản, trong đó 16 còn mở**~~ ~~**[S1.57] 101 khoản, trong đó 15 còn mở**~~ ~~**[S1.58] 102 khoản, trong đó 15 còn mở**~~ ~~**[S1.59] 104 khoản, trong đó 16 còn mở**~~ ~~**[S1.60] 104 khoản, trong đó 15 còn mở**~~ ~~**[S1.61] 105 khoản, trong đó 15 còn mở**~~ ~~**[S1.62] 105 khoản, trong đó 14 còn mở**~~ ~~**[S1.63] 106 khoản, trong đó 14 còn mở**~~ ~~**[S1.64] 107 khoản, trong đó 14 còn mở**~~ ~~**[S1.65] 108 khoản, trong đó 14 còn mở**~~ ~~**[S1.66] 118 khoản, trong đó 24 còn mở**~~ ~~**[S1.67] 119 khoản, trong đó 24 còn mở**~~ ~~**[S1.68] 121 khoản, trong đó 25 còn mở**~~ ~~**[S1.69] 124 khoản, trong đó 27 còn mở**~~ ~~**[S1.70] 125 khoản, trong đó 27 còn mở**~~ ~~**[S1.71] 128 khoản, trong đó 29 còn mở**~~ ~~**[S1.72] 138 khoản, trong đó 38 còn mở**~~ ~~**[S1.73] 140 khoản, trong đó 39 còn mở**~~ ~~**[S1.74] 142 khoản, trong đó 41 còn mở**~~ ~~**[S1.75] 143 khoản, trong đó 41 còn mở**~~ ~~**[S1.76] 146 khoản, trong đó 43 còn mở**~~ ~~**[S1.77] 147 khoản, trong đó 43 còn mở**~~ ~~**[S1.78] 149 khoản, trong đó 45 còn mở**~~ ~~**[S1.79] 152 khoản, trong đó 48 còn mở**~~ ~~**[S1.80] 153 khoản, trong đó 49 còn mở**~~ ~~**[S1.81] 161 khoản, trong đó 57 còn mở**~~ ~~**[S1.82] 167 khoản, trong đó 62 còn mở**~~ ~~**[S1.83] 175 khoản, trong đó 70 còn mở**~~ ~~**[S1.84] 176 khoản, trong đó 69 còn mở**~~ ~~**[S1.85] 177 khoản, trong đó 68 còn mở**~~ ~~**[S1.86] 178 khoản, trong đó 68 còn mở**~~ ~~**[S1.87] 189 khoản, trong đó 79 còn mở**~~ ~~**[S1.90] 196 khoản, trong đó 83 còn mở**~~ ~~**[S1.91] 196 khoản, trong đó 81 còn mở**~~ ~~**[S1.92] 198 khoản, trong đó 82 còn mở**~~ ~~**[S1.93] 202 khoản, trong đó 85 còn mở**~~ ~~**[S1.94] 203 khoản, trong đó 84 còn mở**~~ ~~**[S1.96] 203 khoản, trong đó 82 còn mở**~~ ~~**[S1.97] 204 khoản, trong đó 83 còn mở**~~ ~~**[S1.98] 204 khoản, trong đó 81 còn mở**~~ ~~**[S1.101] 218 khoản, trong đó 85 còn mở**~~ ~~**[S1.103] 218 khoản, trong đó 84 còn mở**~~ ~~**[S1.107] 225 khoản, trong đó 89 còn mở**~~ ~~**[S1.108] 227 khoản, trong đó 91 còn mở**~~ ~~**[S1.109] 230 khoản, trong đó 93 còn mở**~~ ~~**[S1.110] 233 khoản, trong đó 95 còn mở**~~ ~~**[S1.111] 234 khoản, trong đó 96 còn mở**~~ ~~**[S1.112] 235 khoản, trong đó 96 còn mở**~~ ~~**[S1.116] 239 khoản, trong đó 97 còn mở**~~ ~~**[S1.118] 239 khoản, trong đó 96 còn mở**~~ ~~**[S1.118] 239 khoản, trong đó 95 còn mở**~~ ~~**[S1.118] 239 khoản, trong đó 94 còn mở**~~ ~~**[S1.118] 239 khoản, trong đó 93 còn mở**~~ ~~**[S1.129] 239 khoản, trong đó 92 còn mở**~~ ~~**[S1.132] 239 khoản, trong đó 91 còn mở**~~ **[S1.140] 242 khoản, trong đó 93 còn mở** |
| `docs/PRODUCT.md` | Định vị, phạm vi, **những điều không được tuyên bố** |
| `docs/TIEN-DE-CHUA-DO.md` | ~~**17 tiền đề**~~ **[S1.78] 18 tiền đề về con người và quy trình mà S1 đang cư xử như thật** — mỗi dòng một địa chỉ trong kho và một câu hỏi cho người mua thật. Không thay pilot; nó hạ chi phí buổi đầu |
| `docs/ARCHITECTURE.md` | Kiến trúc: modular monolith, `unseal-worker` giữ độc quyền giải mã, RLS đa tổ chức |
| `docs/DECISIONS.md` | ~~**Chín ADR** — 001–008 *Đã chấp nhận*, **009 (KMS) *Đang mở*, chặn S1.6**~~ → ~~**Mười hai ADR**~~ ~~**Mười lăm ADR**~~ ~~**Mười tám ADR**~~ ~~**[S1.28] 34 ADR**~~ ~~**[S1.29] 35 ADR**~~ ~~**[S1.32] 36 ADR**~~ ~~**[S1.43] 37 ADR**~~ ~~**[S1.74] 38 ADR**~~ ~~**[S1.76] 39 ADR**~~ ~~**[S1.82] 40 ADR**~~ ~~**[S1.84] 41 ADR**~~ ~~**[S1.86] 42 ADR**~~ ~~**[S1.88] 43 ADR**~~ ~~**[S1.89] 44 ADR**~~ ~~**[S1.90] 45 ADR**~~ ~~**[S1.91] 46 ADR**~~ ~~**[S1.92] 47 ADR**~~ ~~**[S1.93] 48 ADR**~~ ~~**[S1.100] 49 ADR**~~ ~~**[S1.101] 50 ADR**~~ ~~**[S1.102] 51 ADR**~~ ~~**[S1.104] 52 ADR**~~ ~~**[S1.105] 53 ADR**~~ ~~**[S1.107] 54 ADR**~~ ~~**[S1.108] 55 ADR**~~ ~~**[S1.109] 56 ADR**~~ ~~**[S1.110] 57 ADR**~~ ~~**[S1.111] 58 ADR**~~ ~~**[S1.113] 59 ADR**~~ ~~**[S1.116] 60 ADR**~~ ~~**[S1.118] 61 ADR**~~ ~~**[S1.119] 62 ADR**~~ ~~**[S1.120] 63 ADR**~~ ~~**[S1.121] 64 ADR**~~ ~~**[S1.122] 65 ADR**~~ ~~**[S1.123] 66 ADR**~~ ~~**[S1.124] 67 ADR**~~ ~~**[S1.125] 68 ADR**~~ ~~**[S1.126] 69 ADR**~~ ~~**[S1.127] 70 ADR**~~ ~~**[S1.128] 71 ADR**~~ ~~**[S1.130] 72 ADR**~~ ~~**[S1.131] 73 ADR**~~ ~~**[S1.132] 74 ADR**~~ ~~**[S1.133] 75 ADR**~~ ~~**[S1.134] 76 ADR**~~ ~~**[S1.135] 77 ADR**~~ ~~**[S1.136] 78 ADR**~~ ~~**[S1.137] 79 ADR**~~ ~~**[S1.139] 82 ADR**~~ **[S1.141] 84 ADR**: ~~001–010 và 012–018 *Đã chấp nhận* (009 chốt **AWS KMS**); **011** ***Đang mở***, chặn S1.4/S1.5.~~ ~~**[S1.79] cả 39 ADR đều *Đã chấp nhận***~~ ~~**[S1.82] cả 40 ADR đều *Đã chấp nhận***~~ ~~**[S1.84] cả 41 ADR đều *Đã chấp nhận***~~ ~~**[S1.86] cả 42 ADR đều *Đã chấp nhận***~~ **[S1.100] mọi ADR đều *Đã chấp nhận*** — con số KHÔNG nhắc lại ở đây, vì nó đã có đúng một chỗ được cổng canh (lời khai `42` sống từ S1.87 tới S1.99 — bảy ADR mới mà không ai sửa dòng này; đúng họ với khoản 212, và là chỗ thiu thứ HAI mà vòng S1.100 phải sửa tay) (ADR-042 — người giữ khoá ghi sổ: thu hồi quyền đóng đường CỐ Ý, đường HỢP LỆ được NHẬN và ghi thành khoản 178 — **[S1.87] ĐỌC LẠI:** trục chia đúng là **khoá mức PHIÊN / khoá mức GIAO DỊCH**, không phải *cố ý / hợp lệ*; kẻ cố ý chọn được cơ chế, nên nửa còn mở của khoản 178 bao gồm cả đường cố ý) (ADR-040 — nguồn danh sách tổ chức cho worker) — 009 chốt **AWS KMS**, và **011 chốt TRỌN VẸN 2026-09-04** ở S1.5 (P-256 mặc định, X25519 cơ hội; mục 2 và mục 3 chốt CÙNG NGÀY, gỡ chặn S1.5). Lời khai *"011 đang mở"* sống từ **S1.5 tới S1.78** — qua 82 commit sửa chính tệp này, trong khi chính ADR ấy tự ghi *"ĐÃ CHẤP NHẬN TRỌN VẸN"* suốt thời gian đó. **013/014/015** là ba quyết định của ba hạng mục sớm nhất: phạm vi sổ NCC, nơi cưỡng chế máy trạng thái RFQ, kênh OTP + nền giới hạn tần suất. **016/017/018** là ba quyết định của ba MEDIUM mà vòng sửa an ninh cố ý không đóng bằng mã: cổng quyền ở tầng ứng dụng + danh tính là dẫn xuất, chính sách tính `requires_dual_approval`, pepper cho băm đích |
| `docs/TEST-PLAN.md` | ~~**Sổ đăng ký 47 bất biến** (34 nghiệp vụ + 13 hàng rào)~~ ~~**[S1.28] Sổ đăng ký 55 bất biến** (34 nghiệp vụ + 21 hàng rào)~~ ~~**[S1.29] Sổ đăng ký 56 bất biến** (34 nghiệp vụ + 22 hàng rào)~~ ~~**[S1.115] Sổ đăng ký 62 bất biến** (40 nghiệp vụ + 22 hàng rào)~~ **[S1.116] Sổ đăng ký 63 bất biến** (41 nghiệp vụ + 22 hàng rào) — nguồn sự thật duy nhất |
...
- **[S1.142] Số hiệu cấp lúc merge, không cấp lúc viết (ADR-084; luật S1.111 đã bỏ).** Mục mới trên
