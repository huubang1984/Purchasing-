...
describe("phủ RLS", () => {
  ...
  it("quyền bảng của app_api/app_unseal đúng bằng những gì đã quyết định, không hơn", async () => {
    ...
    expect(rows).toEqual([
      ...
      // [S1.124 / khoản 233 / 064] Trigger J3 đọc lịch sử điều phối dưới quyền người đề xuất.
  ...
  it("[M5] quyền CỘT của app_api/app_unseal đúng bằng những gì đã quyết định", async () => {
    ...
    expect(rows).toEqual([
      ...
      // [S1.124 / khoản 233 / 064] Lịch sử điều phối: trigger ghi chạy dưới quyền người điều phối nên
...
const POLICY_RESTRICTIVE_DA_KHAI: Readonly<Record<string, PolicyRestrictiveKhai>> = (() => {
  ...
  return Object.fromEntries([
    ...[
      ...
      // [S1.124 / khoản 233 / 064] Nhà cung cấp không có việc gì với việc ai đã điều phối mở thầu.
