...
describe("migration của dự án", { timeout: 180_000 }, () => {
  ...
  const HAM_56: readonly { ham: string; migration: string; trigger: readonly string[] }[] = [
    ...
    // [S1.122 / khoản 233] `064` định nghĩa lại thân (vế 3 đọc `unseal_dispatch_history`), nên con
    ...
    { ham: "bid_kiem_han_nop", migration: "065_han_nop_mang_gio_phan_xu.sql", trigger: ["vendor_bid_versions_kiem_han_nop"] },
    ...
    // [S1.122 / khoản 233] Lớp GHI của lịch sử điều phối — nguồn dữ liệu duy nhất của J3 vế 3. Một
  ...
  it("[Minor] migrate() dưới search_path thù địch vẫn đặt lược đồ vào public và vẫn idempotent", async () => {
    ...
    try {
      ...
      try {
        ...
        "065_han_nop_mang_gio_phan_xu.sql",
        ]);
